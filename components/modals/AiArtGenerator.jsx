...
import {
  reducer as voiceReducer,
  initialState as voiceInitialState,
} from '../../lib/utils/reducers/voiceReducer';
import {
...
const AiArtGenerator = observer(({ startUpload, options }) => {
  ...
  const languages = React.useMemo(
    () => (onlyLimitedTextToSpeech ? LIMITED_LANGUAGES : UNLIMITED_LANGUAGES),
    [onlyLimitedTextToSpeech]
  );

  const userVoices = React.useMemo(
    () => (onlyLimitedTextToSpeech ? LIMITED_VOICES : UNLIMITED_VOICES),
    [onlyLimitedTextToSpeech]
  );
  ...
  useEffect(() => {
    if (voiceTextId && symbols) {
      ...
      if (
        typeof voiceTextId === 'object' &&
        voiceTextId.id &&
        voiceTextId.textId
      ) {
        const element = findElement(voiceTextId.id);
        if (element.type === POPCORN_ELEMENT_TYPES.COMBINED) {
          activeTextElement = element.popcornOptions.items.find(
            (el) => el.id === voiceTextId.textId
          );
        }
      } else {
      ...
      const isPersonalizedVoice =
        activeTextElement.text && activeTextElement.text.indexOf('{{') !== -1;
      const maxVoiceSymbols = maxCount(
        isPersonalizedVoice ? maxSymbols.personalized : maxSymbols.text
      );
      const newTextLength = activeTextElement.text.replace(
        /{{\w+}}/g,
        ''
      ).length;
      ...
      if (isPersonalizedVoice) {
        ...
        textArray[lastItemIndex] = textArray[lastItemIndex].slice(
          0,
  ...
  const quantify = () => {
    userCutOutProBalance().catch(() =>
      showError(ERROR_CUTOUTPRO_TEXT_SYMBOLS.title)
    );
  };
  ...
  const processAIImage = async (val) => {
    ...
    await fetch(
      `https://www.cutout.pro/api/v1/getText2imageResult?taskId=${val}`,
      {
        method: 'get',
        headers: {
      ...
      .then(async (resp) => {
        if (resp?.code == 9010) {
          setIsLoading(false);
          setError(
            'The description information has failed the review and contains illegal content'
          );
        }
      ...
      <div className="ai-art-generator__body">
        ...
        <div className="ai-art-generator__content">
            ...
            <div>
              ...
              <div>
                <button
                  ...
                  className={activeButton === 'character' ? 'active' : ''}
                >
                  Character Style
                </button>
                <button
                  onClick={() => setActiveButton('scenario')}
                  className={activeButton === 'scenario' ? 'active' : ''}
                >
                  Scenario Style
                </button>
              </div>
          ...
          <div className="ai-art-generator__content-second">
            ...
            {isProcessImage && <hr />}
            {isProcessImage && (
              <p>Generate an image of the Sci-fi scene set in future world.</p>
            )}
            {!isProcessImage && !isLoading && (
              <p>
                No Image Generated. Please Generate Image Using Prompt to check
                the Results.
              </p>
            )}
            {isLoading && (
