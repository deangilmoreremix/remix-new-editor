...
export const rgbFade = (rgb, opacity, isSubtraction = false) => {
  if (!rgb.match(/^rgba?[\s+]?\([\s+]?(\d+)[\s+]?,[\s+]?(\d+)[\s+]?,[\s+]?(\d+)[\s+]?/i)) {
    fade(rgb, opacity);
  } else {
    const { r, g, b, a } = parseStringRgb(rgb);
    return `rgba(${r}, ${g}, ${b}, ${isSubtraction ? (a - opacity) : opacity})`;
  }
};
...
export const parseStringRgb = (rgba) => {
  rgba = rgba.match(/^rgba?[\s+]?\([\s+]?(\d+)[\s+]?,[\s+]?(\d+)[\s+]?,[\s+]?(\d+)[\s+]?,[\s+]?(\d{0,1}[,.]\d{0,2}|\d{0,1})?/i);
  ...
  return (rgba && rgba.length === 5) ? rgbObject : '';
};
