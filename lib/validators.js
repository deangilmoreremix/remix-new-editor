...
import { EMAIL_REGEX, PHONE_URL_REGEX, TYPES, WEBHOOK_REGEX } from './constants/validator';
import { HINTS } from './constants/text-info';
...
export const wrongEmail = ({
  message = `Incorrect email. ${HINTS.LINK_EMAIL}`,
  value,
  pattern = EMAIL_REGEX,
}) => wrongPattern({ message, value, pattern });

export const wrongPhoneOrUrl = ({
  message = `Incorrect phone or url. ${HINTS.LINK_URL_PHONE}`,
  value,
  pattern = PHONE_URL_REGEX,
}) => wrongPattern({ message, value, pattern });
...
export default {
  ...
  [TYPES.EMAIL]: wrongEmail,
  [TYPES.PHONE_URL]: wrongPhoneOrUrl,
};
