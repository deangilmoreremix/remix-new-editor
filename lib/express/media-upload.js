...
const parseTempFiles = (req) => {
  ...
  if (req.files) {
    paths = Object.keys(req.files).map(field => [req.files[field]].map(file => file.path)).reduce(
...
module.exports.mediaUpload = (req, res) => {
  function uploadFromFile() {
    ...
    return Promise
      ...
      .then(([mediaResponse, preview]) => {
        cleanUpTempFiles(parseTempFiles(req));
        return res.json(Object.assign(mediaResponse, { preview }));
      })
