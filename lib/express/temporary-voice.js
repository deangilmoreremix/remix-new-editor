...
module.exports.getGoogleCloudVoice = async (req, res) => {
  ...
  if (req.body.pitch === undefined) {
    return res.json(400, {
      error: 'invalid_pitch',
      message: `Pitch is: ${req.body.pitch}`,
    });
  }

  if (req.body.speakingRate === undefined) {
    return res.json(400, {
      error: 'invalid_speaker_rate',
      message: `Speaker Rate is: ${req.body.speakingRate}`,
    });
  }
  ...
  const { text, name, languageCode, pitch, speakingRate } = req.body;
