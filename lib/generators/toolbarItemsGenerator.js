...
export const elementItems = ({
  ...
  },
  {
    ...
    label: isfeatureEnabled(FEATURES.REVOLUTION_LB_INTEGRATIONS) ? 'List Builder' : 'Advanced Optin',
    icon: svgListBuilder,
    disabled: false,
    hidden: !isSuperAdmin
  ...
  {
    action: () => {
      actions.openOverlay(WINDOW_TYPES[`${width}:${height}`].value);
    },
    label: 'Overlays',
    icon: svgOverlay,
  ...
  // },
  {
    action: () => actions.openCTA(WINDOW_TYPES.CTA),
    label: 'CTA',
    icon: svgCTA,
    disabled: !ctaEnabled,
  },
  ...
  {
    action: () => actions.openBlendMode(WINDOW_TYPES.BLEND_MODE_LIBRARY),
    label: 'Blend Modes',
    icon: svgBlendModes,
