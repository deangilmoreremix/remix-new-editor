...
export const emailCode = (url, width, height, thumbnail, needPlayButton) => `<a href=${url}><div class='embed-email' style='background-image: url(${`${thumbnail}`}); width: ${`${width}px`}; height: ${`${height}px`}; ${EMAIL_STYLES}'>${needPlayButton ? `<div class="embed-play-button" style="${PLAY_BUTTON_STYLES} background-image: url('https://cdn.vidcloud.io/revolution/resources/play_button.png')"><p style="opacity: 0">&nbsp;</p></div>` : '<p style="opacity: 0">&nbsp;</p>'}</div></a>\r`;
