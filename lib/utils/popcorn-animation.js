...
const animationValuesIdle = animations[ANIMATION_TYPES.IDLE].map(item => item.value);
  ...
  if (options.animation.in) {
    ...
    if (trackEvent.animation.idle) {
      container.addEventListener('animationend', (e) => {
        const { animationName } = e;
        if (options.animation.in.type === animationName) {
...
export const updateOut = (trackEvent, options) => {
  if (options.animation.out) {
    ...
    if (trackEvent.animation.idle) {
      container.addEventListener('animationend', (e) => {
        const { animationName } = e;
        if (options.animation.out.type === animationName) {
