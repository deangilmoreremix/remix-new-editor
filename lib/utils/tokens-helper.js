...
export const unwrapSvgTokens = (string) => {
  if (!string) {
    return '';
  }
  const tokenRegexPattern = '<tspan contenteditable="false" class="(token-default|token-uppercase|token-none|svg-token-none)">([^{}="<>]*)</tspan>';
  const smartTokenRegex = new RegExp(tokenRegexPattern, 'gm');
  return string.replace(smartTokenRegex, (match) => {
    const [, , token] = new RegExp(tokenRegexPattern, 'gm').exec(match);
    return `{{${token}}}`;
  });
};
...
export const wrapSvgTokens = (string) => {
  ...
  return string.replace(TOKEN_REGEX, (match) => {
    ...
    let result;
    if (match.split(' ').length > 1) {
      const [helper, tokenName] = match.split(' ');
      result = `<tspan text-anchor="middle" alignment-baseline="central" class="svg-${TOKEN_HELPER_CLASSES[helper]}">${tokenName}</tspan>`;
    } else {
      result = `<tspan contenteditable="false" class="svg-token-none">${match}</tspan>`;
    }
    result += ' ';
    return result;
  });
