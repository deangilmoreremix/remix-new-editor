    ...
    if (match.split(' ').length > 1) {
      ...
      params.forEach(param => {
        ...
        if (param[param.length - 1] === '"') {
          param = param.slice(param.length - 1);
        }
      });
      ...
      result += `<span class="${TOKEN_HELPER_CLASSES[helper]}" data-parameter="${encodeURIComponent(JSON.stringify(params))}"></span>`;
      result += tokenName;
    } else {
...
export const unwrapTokens = (string) => {
  ...
  return string.replace(smartTokenRegex, (match) => {
    ...
    if (helper && helper !== 'token-none') {
      ...
      if (params) {
        const decodeParams = JSON.parse(decodeURIComponent(params));
        decodeParams.forEach(param => {
          if (param[0] === '"') {
            param = param.slice(1);
          }
        ...
        parsedParams = decodeParams.join(' ');
      }

      return `{{${tokenHelper} ${token}${(parsedParams && parsedParams.length > 0) ? ` ${parsedParams}` : ''}}}`;
    } else {
      return `{{${token}}}`;
    }
  });
...
export const wrapSvgTokens = (string) => {
  ...
  return string.replace(TOKEN_REGEX, (match) => {
    ...
    if (match.split(' ').length > 1) {
      ...
      result += `<tspan class="${TOKEN_HELPER_CLASSES[helper]}" data-parameter="${encodeURIComponent(JSON.stringify(params))}"></tspan>`;
      result += tokenName;
    } else {
