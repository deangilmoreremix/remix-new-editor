...
const mediaTypeDetector = new MediaTypeDetector();
...
export const isAudio = (options = {}) => (options.type === 'SoundCloud')
  || (options.contentType && (options.contentType.indexOf('audio') === 0
    || options.contentType.indexOf('application/ogg') === 0)) || options.kind === 'voice'
  || options.kind === 'personalized-voice';

export const removeVideos = (projectData) => {
  projectData.media.forEach((media) => {
    media.tracks = media.tracks.slice().filter((track) => {
      track.trackEvents = track.trackEvents.slice()
        .filter((item) => item.type !== POPCORN_ELEMENT_TYPES.SEQUENCER
        || (item.type === POPCORN_ELEMENT_TYPES.SEQUENCER && isAudio(item.popcornOptions)));
      return track.trackEvents.length;
    });
  });
};

export const recompressProject = (projectData, newDuration, updateElements) => {
  projectData.media.forEach((media) => {
    ...
    if (updateElements) {
      ...
      media.tracks.forEach((track) => {
        track.trackEvents.forEach((trackEvent) => {
          if (trackEvent.type !== 'sequencer' && trackEvent.type !== 'json-transition') {
...
export const setOptions = async (item, type) => {
  switch (type) {
    case POPCORN_ELEMENT_TYPES.SEQUENCER: {
      const source = (item.extra && item.extra.source) || [item.url];
      const fileDuration = (item.extra && item.extra.duration) || item.duration || null;
      try {
        const fileMeta = await mediaTypeDetector
          .getMetadata(source[0], item.kind === 'audio' ? 'audio' : 'video', fileDuration);
        item.duration = item.duration || (fileMeta.duration === Infinity ? 5 : fileMeta.duration);
        item.fileMeta = fileMeta;
        item.type = fileMeta.type;
        item.source = source;
        return item;
      } catch (e) {
        item.hasError = true;
...
export const getNicheTextData = (item) => {
  const items = [];

  item.media.forEach((media) => {
    media.tracks.forEach((track) => {
      track.trackEvents.forEach((trackEvent) => {
        if (trackEvent.type === 'text' || trackEvent.type === 'combined') {
          items.push(trackEvent.popcornOptions.text);
        }
      });
    });
  });
