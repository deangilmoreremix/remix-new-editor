...
export default class PixabayProvider extends Provider {
  ...
  prepareResults(response) {
    ...
    if (this.assetType === supportedAssetTypes.IMAGE) {
      ...
      return response.hits.map(data => {
        const [tag] = Array.isArray(data.tags) ? data.tags : data.tags.split(',');
        const newTag = tag || `Video by ${data.user}`;
        const title = `${newTag[0].toUpperCase() + newTag.slice(1)}`;
        return {
          _id: `pixabay-${data.id}`,
          ...
          poster: `https://i.vimeocdn.com/video/${data.picture_id}_100x75.jpg`,
          title,
        };
      });
    }
