...
export const reducer = (state, action) => {
  ...
  switch (action.type) {
    case ACTION_TYPES.INIT: {
      ...
      const { language, voices = [], allowedPro, pitch, speakingRate } = action.value;
    ...
    case ACTION_TYPES.SET_PITCH: {
      const { value } = action;
      return { ...state, pitch: +value };
    }
    case ACTION_TYPES.SET_SPEAKING_RATE: {
      const { value } = action;
      return { ...state, speakingRate: +value };
    }
    default:
      return state;
  }
