...
class SendsparkPersonalizationEngine {
  ...
  // Capture website screenshot using PageShot (free, no API key)
  async captureWebsiteScreenshot(url) {
    console.log('  📸 Capturing website screenshot via PageShot...');

    try {
      // PageShot is free with no API key required
      const screenshotUrl = `https://pageshot.site/v1/screenshot?url=${encodeURIComponent(url)}&width=1920&height=1080&format=png&block_ads=true&hide_banners=true`;

      // Fetch the screenshot
      const response = await fetch(screenshotUrl);

      if (!response.ok) {
        throw new Error(`PageShot failed: ${response.statusText}`);
      }

      // Convert to blob
      const blob = await response.blob();

      // Upload to Muapi storage for persistent URL
      const uploadedUrl = await this.muapi.uploadFile(new File([blob], 'background.png', { type: 'image/png' }));
      ...
      console.warn('  ⚠️ PageShot failed:', error.message);
      throw error;
    }
  }
  ...
  // Uses lip-sync to animate user's video with personalized audio on dynamic background
  async synthesizePersonalizedVideo({ baseVideo, audio, background, contact }) {
    console.log('  🎥 Synthesizing personalized video...');
    console.log('     - Using background:', background.type);
    console.log('     - Audio duration:', audio.duration, 'seconds');

    let finalVideoUrl = baseVideo.url;

    // Step 1: If we have a dynamic background (not the original), composite the user onto it
    if (background.type !== 'original') {
      console.log('  🎨 Compositing user onto dynamic background...');

      // First, create a frame from the user's video (first frame or key frame)
      // In production, this would extract a clean plate of the user
      const userImageUrl = baseVideo.thumbnail || baseVideo.url;

      // Use image-to-video with lip-sync on the dynamic background
      // This creates a video of the user speaking on the new background
      const lipSyncResult = await this.muapi.generateLipSync({
        model: 'ltx-2.3-lipsync',
        image_url: userImageUrl,  // User's image/frame
        audio_url: audio.url,      // Personalized audio
        resolution: '1920x1080'
      });

      finalVideoUrl = lipSyncResult.url;
    } else {
      // Use original background, just apply lip-sync
      console.log('  🎙️ Applying lip-sync to original video...');

      const lipSyncResult = await this.muapi.generateLipSync({
        model: 'ltx-2.3-lipsync',
        video_url: baseVideo.url,
        audio_url: audio.url,
        resolution: '1920x1080'
      });
    ...
    // Step 2: Apply video enhancements
    console.log('  ✨ Applying video enhancements...');
    const enhancedResult = await this.enhancer.enhanceVideo(finalVideoUrl, [
      'color_grading',
