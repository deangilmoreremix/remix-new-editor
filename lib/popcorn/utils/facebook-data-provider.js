...
define(['jquery'], ($) => {
  ...
  function postToFeed(options, callback) {
    FB.ui({
      method: 'share',
      href: `${`${window.location.protocol}//${prefix}api.${hostname}`}/api/makes/fb-share/${facebookSocialId
      }?projectUrl=${`${window.location.protocol}//${prefix}projects.${hostname}`
  ...
  function updateCache(data, callback) {
    ...
    return $.post(`${window.location.protocol}//${prefix}api.${hostname}/api/makes/update-fb-cache`, data, callback);
  }
