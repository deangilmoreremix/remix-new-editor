...
export function draggableResizable(
  element,
  callback,
  { restrictBorders = false, edges = {}, ratio = null } = {},
  isScale,
) {
  ...
  function setResizableHandles(isInitialScale) {
    if (isInitialScale) {
      return;
    }
    ...
    deleteButton.addEventListener('mousedown', addEvent => {
      deleteItem(addEvent, element.id, true);
      deleteButton.removeEventListener('mousedown', deleteEvent => deleteItem(deleteEvent, element.id, true));
    });
  ...
  const dragMoveListener = (event) => {
    ...
    const {
      width: canvasWidth,
      height: canvasHeight,
    } = element._container.parentNode.getBoundingClientRect();
    element._container.style.transform = '';
    const {
      width: containerWidth,
      height: containerHeight,
    } = element._container.getBoundingClientRect();

    const x = (element.left / 100) * canvasWidth
      + (isResize ? event.deltaRect.left : event.dx);
    const y = (element.top / 100) * canvasHeight
      + (isResize ? event.deltaRect.top : event.dy);

    const relativeTop = (y / canvasHeight) * 100;
    const relativeLeft = (x / canvasWidth) * 100;
    const relativeHeight = ((isResize ? (containerHeight + event.deltaRect.height)
      : containerHeight) / canvasHeight || 1) * 100;
    const relativeWidth = ((isResize ? (containerWidth + event.deltaRect.width)
      : containerWidth) / canvasWidth || 1) * 100;

    if (relativeTop > 0 - relativeHeight
      && relativeTop < 100) {
      element.top = relativeTop;
      element._container.style.top = '25%';
    }
    if (relativeLeft > 0 - relativeWidth
      && relativeLeft < 100) {
      element.left = relativeLeft;
      element._container.style.left = '25%';
    }
    ...
    element._container.style.transform = `rotate(${element.rotation || 0}deg)`;
  };
  ...
  if (ratio) {
    defaultResizableModifiers.push(
