...
export function draggableResizable(element, callback, { restrictBorders = false } = {}) {
  ...
  const dragMoveListener = (event) => {
    ...
    const x = (parseFloat(target.getAttribute('data-x'))
      || (element.left / 100) * element._container.parentNode.offsetWidth)
      + (isResize ? event.deltaRect.left : event.dx);
    const y = (parseFloat(target.getAttribute('data-y'))
      || (element.top / 100) * element._container.parentNode.offsetHeight)
