...
(function (Popcorn) {
  ...
  function setupIframe(options) {
    ...
    if (options.src) {
      ...
      if (src) {
        ...
        const api = (options._sketchfabAPI = new Sketchfab(iframe));
        ...
        SketchfabQuery.when(api.load(src, sketchfabOptions))
          .then(() => {
  ...
  Popcorn.plugin(
    'sketchfab',
    {
      ...
      _update(trackEvent, options) {
        let resetIframe = false;

        if (
          options.transition &&
          options.transition !== trackEvent.transition
        ) {
        ...
        if (
          [true, false].indexOf(options.background) > -1 &&
          options.background !== trackEvent.background
        ) {
        ...
        if (
          [true, false].indexOf(options.autospin) > -1 &&
          options.autospin !== trackEvent.autospin
        ) {
  ...
  require(['sketchfab-iframe'], (SketchfabIframe) => {
