    ...
    return {
      _setup(element) {
        ...
        ['backgroundColor', 'buttonBorderRadius', 'captionFontSize', 'innerWidth', 'innerHeight',
          ...
          'popupHeaderText', 'popupFillOutText', 'popupClickText', 'popupDescriptionText',
        ].forEach(fillOption);
        ...
        titleHint.classList.add('title-hint');
        tryNowField.classList.add('try-now-hint');
        experienceField.classList.add('experience-hint');

        titleHint.innerText = element.options.popupHeaderText
          ?? element.manifest.options.popupHeaderText.default;
        const spanElementField = element.options.elements.map(el => `<span class="element-field">${el.label}</span>`);
        fillOutField.innerHTML = `<span class="fill-out">${element.options.popupFillOutText ?? element.manifest.options.popupFillOutText.default}</span>${spanElementField}`;
        tryNowField.innerHTML = `${element.options.popupClickText ?? element.manifest.options.popupClickText.default} <span class="btn-form-name">${element.options.btnText ?? element.manifest.options.btnText.default}</span>`;
        experienceField.innerText = element.options.popupDescriptionText
          ?? element.manifest.options.popupDescriptionText.default;
      ...
      start(element) {
        ...
        if (overlayArrowHint && overlayArrowHint.classList.contains('off') && containerFillOut && btnFormName) {
          const spanElementField = element.options.elements.map(el => `<span class="element-field">${el.label}</span>`);
          containerFillOut.innerHTML = `<span class="fill-out">${element.options.popupFillOutText}</span>${spanElementField}`;
      ...
      _update(element, options) {
        ...
        function showOverlayArrowHint() {
          ...
          if (overlayArrowHint.classList.contains('off')) {
            overlayArrowHint.classList.remove('off');
            overlayArrowHint.classList.add('on');
          }
        }
        ...
        if (element._container) {
          ...
          const fillOutHint = document.querySelector('.container-fill-out');
          const tryNowHint = document.querySelector('.try-now-hint');
          const experienceHint = document.querySelector('.experience-hint');
          const spanElementField = element.options.elements.map(el => `<span class="element-field">${el.label}</span>`);
          ...
          if (options.hasOwnProperty('popupClickText')) {
            element.options.popupClickText = options.popupClickText;
            tryNowHint.innerHTML = `${element.options.popupClickText || element.manifest.options.popupClickText.default} <span class="btn-form-name">${element.options.btnText ?? element.manifest.options.btnText.default}</span>`;
            showOverlayArrowHint();
          }
          if (options.hasOwnProperty('popupHeaderText')) {
            element.options.popupHeaderText = options.popupHeaderText;
            titleHint.innerText = element.options.popupHeaderText
              || element.manifest.options.popupHeaderText.default;
            showOverlayArrowHint();
          }
          if (options.hasOwnProperty('popupFillOutText')) {
            element.options.popupFillOutText = options.popupFillOutText;
            fillOutHint.innerHTML = `<span class="fill-out">${element.options.popupFillOutText || element.manifest.options.popupFillOutText.default}</span>${spanElementField}`;
            showOverlayArrowHint();
          }
          if (options.hasOwnProperty('popupDescriptionText')) {
            element.options.popupDescriptionText = options.popupDescriptionText;
            experienceHint.innerHTML = `<span>${element.options.popupDescriptionText || element.manifest.options.popupDescriptionText.default}</span>`;
            showOverlayArrowHint();
          }
  ...
  },
  {
    ...
    options: {
      ...
      [POPUP_HEADER_TEXT]: {
        ...
        type: 'text',
        default: 'Create your very own personalized video',
      },
      [POPUP_FILL_OUT_TEXT]: {
        name: POPUP_FILL_OUT_TEXT,
        label: 'Pop-up Fill Out Text',
        type: 'string',
        default: 'Fill out your',
      },
      [POPUP_CLICK_TEXT]: {
        name: POPUP_CLICK_TEXT,
        label: 'Pop-up Click Text',
        type: 'text',
        default: 'Click',
      },
      [POPUP_DESCRIPTION_TEXT]: {
        name: POPUP_DESCRIPTION_TEXT,
        label: 'Pop-up Description Text',
        type: 'text',
        default: 'Experience the magic of the personalization',
      },
      [BACKGROUND_IMAGE]: {
