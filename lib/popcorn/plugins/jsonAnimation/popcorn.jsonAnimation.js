...
import {
  ...
  setAnimationColors, timeInRange, setIdleState,
  ...
  Popcorn.plugin(POPCORN_ELEMENT_TYPES.JSON_ANIMATION, {
    ...
    async _update(trackEvent, options) {
      if (options.url !== undefined && options.url !== trackEvent.url) {
        ...
        const {
          height: canvasHeight,
        } = trackEvent._container.parentNode.getBoundingClientRect();
        const elementRect = trackEvent._container.getBoundingClientRect();
        const height = (elementRect.height * 100) / canvasHeight;
        trackEvent._container.style.height = `${height}%`;
        const props = {};
        if (height) {
          props.height = height;
        }
