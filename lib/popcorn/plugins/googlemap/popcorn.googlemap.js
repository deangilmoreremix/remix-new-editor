/* eslint-disable space-in-parens,no-underscore-dangle,no-new-func,no-multi-assign,prefer-destructuring,no-var,vars-on-top,no-shadow,camelcase,block-scoped-var,no-use-before-define,no-plusplus,no-undef,max-len */
// PLUGIN: Google Maps
/* global google */
...
(function (Popcorn) {
  ...
  const STAMEN_BUTTER_CACHED_URL = '/external/stamen/tile.stamen-1.2.4.js';
  ...
  function buildMap(options, mapDiv, pluginInstance) {
    const type = options.type ? options.type.toUpperCase() : 'ROADMAP';
   ...
   * -Target is the id of the DOM element that you want the map
   * to appear in. This element must be in the DOM
   * -Type [optional] either: HYBRID (default), ROADMAP, SATELLITE,
   * TERRAIN, STREETVIEW, or one of the
   ...
   * -Heading [optional] STREETVIEW orientation of camera in
   * degrees relative to true north (0 north, 90 true east, ect)
   * -Pitch [optional] STREETVIEW vertical orientation of
   * the camera (between 1 and 3 is recommended)
   ...
   * -Location: the address you want the map to display, must be present if lat and lng are
   * not specified.
   * Note: using location requires extra loading time, also not specifying both
   * lat/lng and location will
   * cause and error.
   *
   ...
   * Note that both location and tween must be present when using an
   * auto generated route, or the map will not tween
   * -interval is the speed in which the tween will be executed,
   * a reasonable time is 1000 ( time in milliseconds )
   * Heading, Zoom, and Pitch streetview values are also used in tweening with the
   * autogenerated route
   *
   * -tween is an array of objects, each containing data for one frame of a tween
   * -position is an object with has two parameters, lat and lng, both which are mandatory
   * for a tween to work
   * -pov is an object which houses heading, pitch, and zoom parameters, which are all optional,
   * if undefined, these values default to 0
   * -interval is the speed in which the tween will be executed, a reasonable time is 1000
  ...
  Popcorn.plugin('googlemap', function (options) {
    ...
    let outerdiv; let innerdiv; let map; let location;
        ...
        var isMapSetup = function () {
              ...
              if (typeof options.tween === 'object') {
                for (let i = 0, m = rM.length; i < m; i++) {
                  ...
                  if (current_time >= (waypoint.interval * (i + 1)) / 1000
                    ...
                    sView3.setPosition(new google.maps.LatLng(waypoint.position.lat,
          ...
          if (map && !ranOnce) {
            ...
            if (options.zoom && typeof options.zoom !== 'number') {
              ...
              && +options.zoom <= MAX_MAP_ZOOM_VALUE ? +options.zoom : DEFAULT_MAP_ZOOM_VALUE;
            }
            ...
            if (options.heading && typeof options.heading !== 'number') {
              options.heading = +options.heading >= 0 && +options.heading <= MAX_MAP_HEADING_VALUE
                ? +options.heading : DEFAULT_MAP_HEADING_VALUE;
            }
            // Make sure pitch is a number
            if (options.pitch && typeof options.pitch !== 'number') {
              options.pitch = +options.pitch >= 0 && +options.pitch <= MAX_MAP_PITCH_VALUE
                ? +options.pitch : DEFAULT_MAP_PITCH_VALUE;
            }
            ...
            if (options.type === 'STREETVIEW') {
              ...
              //  Creating another variable to hold the streetview map for tweening,
                //  Doing this because if there was more then one streetview map,
                // the tweening would sometimes appear in other maps
                var sView2 = sView;
                ...
                const directionsDisplay = new google.maps.DirectionsRenderer(sView2);
                ...
                var showSteps = function (directionResult) {
                  //  Push new google map lat and lng values into an array from our
                  // list of lat and lng values
