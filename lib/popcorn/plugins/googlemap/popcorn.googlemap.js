...
const GOOGLE_API_KEY = 'AIzaSyD-PEx7LqWl089hTc91YvRaoI4PssLkdZY';
let googleCallback;
  ...
  Popcorn.plugin('googlemap', function (options) {
    ...
    return {
      _setup(options) {
        ...
        _container.style.width = `${options.fullscreen ? 100 : options.width}%`;
        _container.style.height = `${options.fullscreen ? 100 : options.height}%`;
        _container.style.left = `${options.fullscreen ? 0 : options.left}%`;
        _container.style.top = `${options.fullscreen ? 0 : options.top}%`;
      ...
      start(event, options) {
        var that = this;
        var redrawBug;
        var MAX_MAP_ZOOM_VALUE = 22;
        var DEFAULT_MAP_ZOOM_VALUE = options._natives.manifest.options.zoom.default;
        var MAX_MAP_PITCH_VALUE = 12;
        var DEFAULT_MAP_PITCH_VALUE = options._natives.manifest.options.pitch.default;
        var MAX_MAP_HEADING_VALUE = 12;
        var DEFAULT_MAP_HEADING_VALUE = options._natives.manifest.options.heading.default;
        ...
        var isMapSetup = function () {
          ...
          if (map && !ranOnce) {
            ...
            if (options.location && typeof options.tween === 'string') {
              ...
              var directionsDisplay = new google.maps.DirectionsRenderer(streetDiv);
      ...
      _update(trackEvent, options) {
        ...
        trackEvent._container.style.top = `${trackEvent.fullscreen ? 0 : trackEvent.top}%`;
        trackEvent._container.style.left = `${trackEvent.fullscreen ? 0 : trackEvent.left}%`;
        trackEvent._container.style.height = `${trackEvent.fullscreen ? 100 : trackEvent.height}%`;
        trackEvent._container.style.width = `${trackEvent.fullscreen ? 100 : trackEvent.width}%`;
    ...
    options: {
      ...
      type: {
        elem: 'select',
        options: ['Road Map', 'Satellite', 'Side-By-Side', 'Street View', 'Hybrid', 'Terrain'],
        values: ['ROADMAP', 'SATELLITE', 'SIDEBYSIDE', 'STREETVIEW', 'HYBRID', 'TERRAIN'],
        label: 'Map Type',
        default: 'ROADMAP',
        optional: true,
      },
