...
import { GOOGLE_MAP_VALUES, GOOGLE_MAP_ITEMS } from '../../../constants/googleMap';
...
const LEFT_DEFAULT = 15;
const WIDTH_DEFAULT = 70;
const HEIGHT_DEFAULT = 70;
...
(function (Popcorn) {
  ...
  const onDblClick = (options, _container) => {
  ...
  const removeDbClick = (options, _container) => {
  ...
  Popcorn.plugin('googlemap', function (options) {
    ...
    return {
      _setup(options) {
        ...
        options.left = (options.fullscreen ? 0 : options.left) ?? options._natives.manifest.options.left.default;
        options.top = (options.fullscreen ? 0 : options.top) ?? options._natives.manifest.options.top.default;
        ...
        draggableHandle.addEventListener('dblclick', () => onDblClick(options, _container));
        ...
        const emitterSelectItem = id => {
          ...
          emitter.off(emitterActions.SELECT, emitterSelectItem);
        };
      ...
      _update(trackEvent, options) {
        ...
        if (options.fullscreen !== undefined) {
          ...
          if (!trackEvent.fullscreen) {
            if (!trackEvent.top) {
              trackEvent.top = TOP_DEFAULT;
            }
            if (!trackEvent.left) {
              trackEvent.left = LEFT_DEFAULT;
            }
            if (!trackEvent.width) {
              trackEvent.width = WIDTH_DEFAULT;
            }
            if (!trackEvent.height) {
              trackEvent.height = HEIGHT_DEFAULT;
            }
          }
        }
