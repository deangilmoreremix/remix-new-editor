...
const GOOGLE_API_KEY = 'AIzaSyAfB-p4iXW6Cv61IEAQShIOYPzAqnHtdpk';
let googleCallback;
...
(function (Popcorn) {
  ...
  function buildMap(options, mapDiv, pluginInstance) {
    ...
    // options.oldType - is for side-by-side map (view on left)
    const typeForMap = type === 'SIDEBYSIDE' ? options.oldType : type;

    const map = new google.maps.Map(mapDiv, {
      // If a custom layer was specified, use that, otherwise use type
      mapTypeId: layer || google.maps.MapTypeId[typeForMap || type],
  ...
  Popcorn.plugin('googlemap', function (options) {
    ...
    options.type = options.actionType || GOOGLE_MAP_VALUES.ROADMAP;
    ...
    return {
      _setup(options) {
        ...
        const emitterSelectItem = id => {
          ...
          if (isSelected) {
            draggableResizable({ ...options, _container, type: POPCORN_ELEMENT_TYPES.GOOGLE_MAP }, null);
          } else {
            options.stopMove = false;
      ...
      _update(trackEvent, options) {
        ...
        if (options.type && options.type !== trackEvent.type) {
          oldType = options.oldType || trackEvent.type;
          trackEvent.oldType = options.oldType || trackEvent.type;
    ...
    options: {
      ...
      transition: {
        ...
        items: [
          ...
          { label: 'Fade', value: 'popcorn-fade-in' },
