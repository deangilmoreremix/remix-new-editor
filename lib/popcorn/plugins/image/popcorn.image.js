  ...
  Popcorn.plugin('image', {
    ...
    _setup(options) {
      ...
      if (_target) {
        ...
        if (options.url) {
          if (options.url.indexOf(FLICKR_SINGLE_CHECK) > -1) {
            let url = options.url;
            let ln;
            let photoId;
            let i;
            ...
            _link = createImageDiv(options, options.url);
            setupImageDiv();
          }
        } else {
          let _inOuts = [];
        ...
        options.toString = () => {
          ...
          if (options.title) {
            ...
            // might ba a data URI
            return `${options.url.substring(0, 30)}...`;
          } else if (options.url) {
            _splitSource = options.url.split('/');
    ...
    _update(trackEvent, options) {
      ...
      if ((options.url !== undefined && options.url !== trackEvent.url)
        ...
        imageDiv.style.backgroundImage = `url( "${trackEvent.url}" )`;
