  ...
  Popcorn.plugin('image', {
    ...
    _update(trackEvent, options) {
      ...
      if (options.rotation !== undefined && options.rotation !== trackEvent.rotation) {
        ...
        const { top, left, height, width } = trackEvent._container.style;
        const updateElementProperty = (property, value) => {
          options[property] = value;
          trackEvent[property] = value;
          trackEvent._container.style[property] = `${validateDimension(value, '0')}%`;
        };
        if (parseFloat(top) < 0) {
          updateElementProperty('top', 0);
        }
        if (parseFloat(top) > 100 - parseFloat(height)) {
          updateElementProperty('top', 100 - parseFloat(height));
        }
        if (parseFloat(left) < 0) {
          updateElementProperty('left', 0);
        }
        if (parseFloat(left) > 100 - parseFloat(width)) {
          updateElementProperty('left', 100 - parseFloat(width));
        }
      }
