...
import * as popcornConstants from '../../../constants/popcorn';
...
import { TYPES } from '../../../constants/validator';
import { addMouseDownEvent } from '../../../utils/popcorn-helper';
// const interact = require('interactjs');
import { extendObservable } from 'mobx';
import { animationStart, animationEnd, updateIn, updateIdle, updateOut } from '../../../../lib/utils/popcorn-animation';
...
((Popcorn) => {
  ...
  function getDefaultOptionValue(key) {
    const { options } = Popcorn.manifest[popcornConstants.POPCORN_ELEMENT_TYPES.IMAGE];
  ...
  function createImageDiv(element, imageUrl) {
    ...
    imageDiv.style.backgroundSize = element.fill ? 'cover' : 'contain';
  ...
  Popcorn.plugin('image', {
    ...
    _setup(options) {
      ...
      options.width = options.fill ? 100 : options.width;
      options.height = options.fill ? 100 : options.height;
      options.top = options.fill ? 0 : options.top;
      options.left = options.fill ? 0 : options.left;
      options.stopResize = !!options.fill;
      options.stopMove = !!options.fill;
      ...
      emitter.on(emitterActions.SELECT, id => {
        ...
        if (isSelected) {
          draggableResizable({ ...options, _container }, null);
        } else {
          removeHandleElements(options._container);
        }
      });

      if (!options.start && options.animation.in) {
        options.start = 0.01;
      }
      ...
      _container.style.mixBlendMode = options[popcornConstants.BLEND_MODE]
        || getDefaultOptionValue(popcornConstants.BLEND_MODE);
      _container.style.opacity = options[popcornConstants.OPACITY] !== undefined
        ? `${options[popcornConstants.OPACITY]}%` : `${getDefaultOptionValue(popcornConstants.OPACITY)}%`;

      _container.classList.add(
        options.transition
        || Popcorn.manifest[popcornConstants.POPCORN_ELEMENT_TYPES.IMAGE]
      ...
      options.link.style.borderRadius = options.cornerRadius ? '50%' : 0;
      ...
      extendObservable(options, {
        src: options.src,
        linkSrc: options.linkSrc || getDefaultOptionValue(popcornConstants.LINKSRC),
        ...
        rotation: options.rotation || getDefaultOptionValue(popcornConstants.ROTATION),
        top: +options.top || getDefaultOptionValue(popcornConstants.TOP),
        left: +options.left || getDefaultOptionValue(popcornConstants.LEFT),
    ...
    _update(trackEvent, options) {
      ...
      if (options.animation) {
        ...
        if (!trackEvent.start && options.animation.in) {
          trackEvent.start = 0.01;
        }
      }
      ...
      if (options.fill !== undefined && options.fill !== trackEvent.fill) {
        trackEvent.fill = options.fill;
        trackEvent.width = options.fill ? 100 : trackEvent.width;
        trackEvent.height = options.fill ? 100 : trackEvent.height;
        trackEvent.top = options.fill ? 0 : trackEvent.top;
        trackEvent.left = options.fill ? 0 : trackEvent.left;
        trackEvent.stopResize = !!options.fill;
        trackEvent.stopMove = !!options.fill;
        const imageDiv = trackEvent.link.querySelector('.image-plugin-img');
        imageDiv.style.backgroundSize = trackEvent.fill ? 'cover' : 'contain';
        trackEvent._container.style.width = `${validateDimension(trackEvent.width, '100')}%`;
        trackEvent._container.style.height = `${validateDimension(trackEvent.width, '100')}%`;
        trackEvent._container.style.top = `${validateDimension(trackEvent.top, '0')}%`;
        trackEvent._container.style.left = `${validateDimension(trackEvent.left, '0')}%`;
        draggableResizable({ ...trackEvent, _container: trackEvent._container }, null, {}, true);
      }
      ...
      trackEvent.link.style.borderRadius = trackEvent.cornerRadius ? '50%' : 0;
    ...
    manifest: {
      ...
      options: {
        ...
        [popcornConstants.BLEND_MODE]: {
          default: blendModeConstants.normal.value,
          hidden: true,
        },
        [popcornConstants.OPACITY]: {
          default: 100,
          hidden: true,
        },
        src: {
        ...
        [popcornConstants.TITLE]: {
          type: inputTypes.INPUT,
          label: 'Image Title',
          default: '',
        },
        [popcornConstants.LINKSRC]: {
          type: inputTypes.BLUR_ENTER_INPUT,
          ...
          validationProps: { type: TYPES.PHONE_URL },
          default: '',
        },
        [popcornConstants.CALL_NOTIFY_ADDRESS]: {
          type: inputTypes.BLUR_ENTER_INPUT,
          name: popcornConstants.CALL_NOTIFY_ADDRESS,
          label: 'Email to notify about call attempt',
          className: 'image-settings__field',
          labelClassName: 'image-settings__label--input',
          validationProps: { type: TYPES.EMAIL },
          default: '',
        },
        tags: {
        ...
        [popcornConstants.CORNER_RADIUS]: {
        ...
        [popcornConstants.TOP]: {
          type: inputTypes.NUMBER,
          label: 'Top',
          default: 0,
          hidden: true,
        },
        [popcornConstants.LEFT]: {
          type: inputTypes.NUMBER,
          label: 'Left',
          default: 0,
          hidden: true,
        ...
        [popcornConstants.TRANSITION]: {
          ...
          options: [
            ...
            { label: 'Pan & Zoom', value: 'popcorn-pan-zoom' },
        ...
        [popcornConstants.START]: {
          type: inputTypes.TIME,
          label: 'Start',
          default: 0,
        },
        [popcornConstants.END]: {
          type: inputTypes.TIME,
          label: 'End',
        },
        [popcornConstants.ROTATION]: {
          type: inputTypes.INPUT_ANGLE,
          label: 'Rotation',
          default: 0,
        },
        [popcornConstants.FILL]: {
          type: inputTypes.CHECKBOX,
          label: 'Fill',
        },
        zindex: {
