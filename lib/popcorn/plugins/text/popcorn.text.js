  ...
  Popcorn.plugin(POPCORN_ELEMENT_TYPES.TEXT, {
    ...
    _update(trackEvent, options) {
      ...
      options.runTextfill = () => {
        ...
        const elementRect = trackEvent._container.getBoundingClientRect();
        const resizeOptions = {
          innerTag: 'span',
          maxFontPixels: -1,
          explicitWidth: elementRect.width,
          explicitHeight: elementRect.height,
        };
      ...
      if (options.fontDecorations
        ...
        && !options.fontDecorations.responsive) {
        if (trackEvent) {
          setDragggable(trackEvent, options.runTextfill, true);
        }
      }
      ...
      if (options.fontFamily !== undefined && options.fontFamily !== trackEvent.fontFamily) {
        ...
        if (trackEvent.fontDecorations.responsive) {
          ...
          */
          ...
          setTimeout(options.runTextfill, 30);

          jQuery(window).resize(() => {
            if (trackEvent.resizeTimer) {
              clearTimeout(trackEvent.resizeTimer);
            }
            trackEvent.resizeTimer = setTimeout(options.runTextfill, 30);
          });
        }
      }
