...
import { ADVANCED_GROUP, VRTEXT } from "../../../constants/settings/common";
import { POSITION_CUSTOM } from "../../../constants/settings/vrtext-element";
...
((Popcorn, jQuery) => {
  ...
  function updateDraggableResizable(element, options, textfill) {
    ...
    const dragMoveListener = (event) => {
      const { target } = event;
      const x = (
        parseFloat(target.getAttribute('data-x')) || (element.left / VALUE_CALCULATE)
    * element._container.parentNode.offsetWidth
      )
        + (event.deltaRect ? event.deltaRect.left : event.dx);
      const y = (
        parseFloat(target.getAttribute('data-y')) || (element.top / VALUE_CALCULATE)
    * element._container.parentNode.offsetHeight
      ...
      if (event.rect) {
        ...
        element._container.style.maxHeight = `${VALUE_CALCULATE - element.top}%`;
      }
  ...
  Popcorn.plugin(VRTEXT, {
    ...
    _setup(options) {
      ...
      const top = options.top || options._natives.manifest.options.top.default;
      const text = options.text || options._natives.manifest.options.text.default;
      const linkUrl = options.linkUrl || options._natives.manifest.options.linkUrl.default;
      const shadowColor = options.shadowColor || options._natives.manifest.options.shadowColor.default;
      const backgroundColor = options.backgroundColor || options._natives.manifest.options.backgroundColor.default;
      const strokeColor = options.strokeColor || options._natives.manifest.options.strokeColor.default;
      const rotation = options.rotation || options._natives.manifest.options.rotation.default;
      const context = this;
      const fontDecorations = options.fontDecorations
        || options._natives.manifest.options.fontDecorations.default;
      const padding = '3';
      const width = 100 - (padding * 2);
      let alignment = options.alignment || options._natives.manifest.options.alignment.default;
      let position = options.position || options._natives.manifest.options.position.default;
      let linkUrlSpan;

      options.text = options.text || options._natives.manifest.options.text.default;
      options.fontDecorations = options.fontDecorations
        || options._natives.manifest.options.fontDecorations.default;

      options.left = options.left || options._natives.manifest.options.left.default;
      options.height = options.height || options._natives.manifest.options.height.default;
      options.width = options.width || options._natives.manifest.options.width.default;
      options.top = options.top || options._natives.manifest.options.top.default;
      options.linkUrl = options.linkUrl || options._natives.manifest.options.linkUrl.default;
      ...
      options.fontSize = options.fontSize || options._natives.manifest.options.fontSize.default;
      ...
      options._container.addEventListener('click', (event) => {
        ...
        if (!options.linkUrl) {
          ...
          wrapTokens(options._natives.manifest.options.linkUrl.default);
        }
    ...
    _update(trackEvent, options) {
      ...
      let defaultAlignmentClass = trackEvent._natives.manifest.options.alignment.default;
