...
import { runTextfill, updateTrackEvent } from '../../../utils/popcorn-helper';
...
((Popcorn, jQuery) => {
  ...
  const setDragggable = (options, cb, isScale = false) => {
    let config = {};
    ...
    draggableResizable(options, cb, config, isScale);
  };
  ...
  Popcorn.plugin(POPCORN_ELEMENT_TYPES.TEXT, {
    ...
    _setup(options) {
      ...
      fontSheet.onload = () => {
        ...
        if (options.fontDecorations.responsive) {
          ...
          */
          ...
          setTimeout(() => runTextfill(options), 30);

          jQuery(window).resize(() => {
            if (options.resizeTimer) {
              clearTimeout(options.resizeTimer);
            }
            options.resizeTimer = setTimeout(() => runTextfill(options), 30);
          });
        }
      };
      ...
      if (!options.combined) {
        ...
        emitter.on(emitterActions.SELECT, data => {
          ...
          if (isSelected) {
            setDragggable(options, () => runTextfill(options));
          } else {
    ...
    _update(trackEvent, options) {
      ...
      if (options.fontDecorations
        ...
        && !options.fontDecorations.responsive) {
        if (trackEvent) {
          setDragggable(trackEvent, () => runTextfill(trackEvent), true);
        }
      }
      ...
      if (options.fontFamily !== undefined && options.fontFamily !== trackEvent.fontFamily) {
        ...
        fontSheet.onload = () => {
          ...
          runTextfill(trackEvent);
        };
        document.head.appendChild(fontSheet);
        fontSheet.href = `https://fonts.googleapis.com/css?family=${options.fontFamily.replace(/\s/g, '+')}:400,700`;
        if (options.fontDecorations && options.fontDecorations.responsive) {
          if (trackEvent.fontDecorations.responsive) {
            ...
            */
            ...
            jQuery(window).resize(() => {
              ...
              trackEvent.resizeTimer = setTimeout(() => runTextfill(trackEvent), 30);
            });
          }
        }
      }
      ...
      const innerSpan = trackEvent._contentSpan;
      const link = document.createElement('a');
      ...
      if (trackEvent.fontDecorations.responsive) {
        if (trackEvent.height) {
          ...
          trackEvent._container.firstChild.style.fontSize = 'inherit';
          if (trackEvent._container.firstChild && trackEvent._container.firstChild.firstChild
            && trackEvent._container.firstChild.firstChild.firstChild) {
            trackEvent._container.firstChild.firstChild.firstChild.style.height = '';
          }
        }
        ...
        if (trackEvent._container.firstChild && trackEvent._container.firstChild.firstChild
          && trackEvent._container.firstChild.firstChild.firstChild) {
          trackEvent._container.firstChild.firstChild.firstChild.style.fontSize = 'inherit';
        }
