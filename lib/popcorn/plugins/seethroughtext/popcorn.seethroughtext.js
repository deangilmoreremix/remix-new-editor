...
import { addDeleteListener, removeDeleteListener, selectItem } from '../../../mitt/emitter';
...
((Popcorn) => {
  ...
  const textfillSvg = options => {
    ...
    if (svgText) {
      let textDimensions = svgText.getComputedTextLength();
      do {
        fontSize -= 1;
        outer.style.font = `${options.fontDecorations} ${fontSize}em ${options.fontFamily}`;
        textDimensions = svgText.getComputedTextLength();
      } while ((textDimensions > outer.clientWidth) && (options.text.length > 0) && (fontSize > 0));
    }
  };

  const createText = (parent, options) => {
    const outer = document.createElement('div');
    outer.style.fontFamily = 'inherit';
    Object.assign(outer.style, {
      width: '100%',
      height: '100%',
      font: `${options.fontDecorations.bold} ${options.fontSize}em ${options.fontFamily}`,
    });
  ...
  Popcorn.plugin(POPCORN_ELEMENT_TYPES.TEXT_MASK, {
    ...
    _setup(options) {
      ...
      innerDivCanvas.style.fontFamily = 'inherit';
    ...
    _update(trackEvent, options) {
      ...
      if (options.htmlText && options.htmlText !== trackEvent.htmlText) {
