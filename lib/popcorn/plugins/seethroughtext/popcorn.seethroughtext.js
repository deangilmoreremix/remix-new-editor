...
import { addSelectListener, removeSelectListener } from '../../../mitt/emitter';
...
(function (Popcorn) {
  ...
  function createText(canvas, options) {
    ...
    const vH = v && v.offsetHeight;
    const vW = v && v.offsetWidth;
  ...
  Popcorn.plugin('seethroughtext', {
    ...
    _setup(options) {
      ...
      removeSelectListener(container, options.id);
      addSelectListener(container, options.id);
