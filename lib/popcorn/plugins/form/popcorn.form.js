...
import { BLUR_ENTER_INPUT } from '../../../constants/forms';
  ...
  Popcorn.plugin('form', () => {
    ...
    function buildForm(options) {
      ...
      if (options.innerColor) {
        inner.style.background = fade(options.innerColor,
      ...
      if (options.backgroundImage) {
        ...
        } 10%,${fade(options.backgroundColor, 0.8)} 90%,${
          fade(options.backgroundColor, 1)} 100%)`;
      }
      ...
      ['.buttons-inner > button'].forEach((selector) => {
        form.querySelectorAll(selector).forEach((item) => {
          ...
          item.style.background = options.buttonBackground
            ? fade(options.buttonBackground, 1)
      ...
      _update(trackEvent, options) {
        ...
        if (trackEvent._container) {
          ...
          if (options.hasOwnProperty('backgroundColor')) {
            ...
            form.style.background = `radial-gradient(circle, ${
              fade(trackEvent.backgroundColor, 0.07)} 10%,${
              fade(trackEvent.backgroundColor, 0.8)} 90%,${
              fade(trackEvent.backgroundColor, 1)} 100%)`;
          }
          ...
          if (options.hasOwnProperty('innerColor')) {
            ...
            if (trackEvent.innerColor) {
              inner.style.background = fade(trackEvent.innerColor,
          ...
          if (options.hasOwnProperty('buttonBackground')) {
            ...
            ['.buttons-inner > button'].forEach((selector) => {
              form.querySelectorAll(selector).forEach((item) => {
                item.style.background = fade(trackEvent.buttonBackground, 1);
              });
            });
          }
          if (options.hasOwnProperty('buttonFontColor')) {
            ...
            ['.buttons-inner > button'].forEach((selector) => {
              form.querySelectorAll(selector).forEach((item) => {
                item.style.color = fade(trackEvent.buttonFontColor, 1);
              });
            });
          }
