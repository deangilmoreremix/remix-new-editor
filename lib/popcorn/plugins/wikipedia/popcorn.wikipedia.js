/* eslint-disable space-in-parens,no-underscore-dangle,no-new-func,no-multi-assign,prefer-destructuring,no-var,vars-on-top,no-shadow,camelcase,block-scoped-var,no-use-before-define,no-plusplus,no-undef,max-len,no-throw-literal,no-unused-vars,global-require,import/no-dynamic-require,prefer-const,no-prototype-builtins */

(function (Popcorn) {
  let allWikiLangLinks;
  let allWikiLangNames;
  ...
  function setupWiki(options, instance) {
    ...
    const _guid = Popcorn.guid("wikiCallback");
    ...
    function buildArticle(data) {
      ...
      const textData = data.parse.text["*"];

      const responseFragment = getFragment(`<div>${textData}</div>`);

      let element = responseFragment.querySelector(
        `div > p:nth-of-type(${childIndex})`
      );

      let mainText = "";

      _titleTextArea.appendChild(
        getFragment(
          `<a href="${options._link}" target="_blank">${sanitize(
            data.parse.title
          )}</a>`
        )
      );
      ...
      while (!areValidElements(element)) {
        element = responseFragment.querySelector(
          `div > p:nth-of-type(${++childIndex})`
        );
      }
    ...
    if (options.src) {
      ...
      options._link = `https://${
        options.lang
      }.wikipedia.org/wiki/${encodeURIComponent(_title)}`;
      ...
      if (!cachedArticles[_query]) {
        ...
        Popcorn.getScript(
          `${_href}api.php?action=parse&prop=text&redirects&page=${encodeURIComponent(
            _title
          )}&noimages=1&mobileformat=html&format=json&callback=${_guid}`
        );
      } else {
  ...
  allWikiLangLinks = (
    "en,ja,es,de,ru,fr,it,pt,pl,zh,nl,tr,ar,sv,id,cs,fi,ko,th,fa,hu,he,no,vi,uk,da,ro" +
    ...
    "pnt,ak,dz,st,sd,ik,ts,nso,y,tn,ki,ff,rn,xh,sg,ve,tw,ks,tum,fj,ti,lg"
  ).split(",");

  allWikiLangNames = (
    "English,Japanese,Spanish,German,Russian,French,Italian,Portuguese,Polish," +
    ...
    "Fulfulde,Kirundi,Xhosa,Sangro,Venda,Twi,Kashmiri,Tumbuka,Fijian,Tigrinya,Ganda"
  ).split(",");
