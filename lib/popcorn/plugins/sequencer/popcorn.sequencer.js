...
(function (Popcorn) {
  ...
  const loadingHandler = {
    ...
    next(currentTime) {
      ...
      for (let index = 0; index < this.loading.length; index += 1) {
        if (
          this.loading[index].start <= currentTime &&
  ...
  Popcorn.plugin('sequencer', {
    _setup(options) {
      ...
      options.setupContainer = () => {
        ...
        container.style.mixBlendMode =
          options[BLEND_MODE] || getDefaultOptionValue(BLEND_MODE);
        container.style.opacity =
          options[OPACITY] !== undefined
            ? `${options[OPACITY]}%`
            : `${getDefaultOptionValue(OPACITY)}%`;
      ...
      options.readyEvent = () => {
        ...
        options._clip.media.style.objectFit = options.fill
          ? 'cover'
          : 'contain';
      ...
      options.attemptJWPlayer = () => {
        ...
        // Now we can fail.
        options._clip = new Popcorn(jwplayer, {
          frameAnimation: true,
          framerate: 120,
        });
      ...
      options.addSource = () => {
        ...
        options._clip = Popcorn.smart(options._container, options.source, {
          frameAnimation: true,
          framerate: 120,
        });
        ...
        if (options.active && event && event.detail && event.detail.data) {
          for (let i = 0; i < Popcorn.instances.length; i += 1) {
            if (Popcorn.instances[i].media !== options._clip.media) {
              Popcorn.instances[i].media.currentTime =
                options.start + event.detail.data.seconds;
            }
          }
        }
      };
      ...
      options._onTimeUpdate = () => {
        ...
        if (
          +_this.currentTime().toFixed(2) >
            options.end - options.audioFadeOut &&
          +_this.currentTime().toFixed(2) < options.end &&
          !fade.fadeOut[`isStarted${options.id}`]
        ) {
      ...
      options._setClipCurrentTime = (time) => {
        ...
        if (
          time !== options._clip.currentTime() &&
          time >= +options.from &&
      ...
      options._audioFadeIn = () => {
        ...
        if (
          _this.currentTime() - options.start >= options.audioFadeIn ||
          options.audioFadeIn === undefined ||
          options._clip.paused()
        ) {
          return;
        }

        if (options.audioFadeIn > 0) {
          fade.fadeIn[`time${options.id}`] =
            _this.currentTime() - options.start;
          const startVolume = +(
            (options.volume * fade.fadeIn[`time${options.id}`]) /
            options.audioFadeIn
          ).toFixed(2);
          ...
          fade.fadeIn[`interval${options.id}`] = setInterval(() => {
            if (
              fade.fadeIn[`time${options.id}`] >= options.audioFadeIn ||
            ...
            options._clip.volume(
              fade.fadeIn[`currentVolume${options.id}`] / 100
            );
            fade.fadeIn[`currentVolume${options.id}`] = +(
              fade.fadeIn[`currentVolume${options.id}`] +
              (options.volume * step) / (options.audioFadeIn * 1000)
            ).toFixed(2);
      ...
      options._audioFadeOut = () => {
        ...
        if (
          _this.currentTime() > options.end ||
          options.audioFadeOut === undefined ||
          options._clip.paused()
        ) {
          return;
        }
        ...
        let startVolume = +(
          options.volume -
          (options.volume -
            (options.volume * (options.end - _this.currentTime())) /
              options.audioFadeOut)
        ).toFixed(2);
        ...
        fade.fadeOut[`interval${options.id}`] = setInterval(() => {
          if (
            fade.fadeOut[`time${options.id}`] >= options.end ||
          ...
          options._clip.volume(
            fade.fadeOut[`currentVolume${options.id}`] / 100
          );
          fade.fadeOut[`currentVolume${options.id}`] = +(
            fade.fadeOut[`currentVolume${options.id}`] -
            (options.volume * step) / (options.audioFadeOut * 1000)
          ).toFixed(2);
      ...
      options._playEvent = () => {
        if (options._clip.paused() && !_waiting && !options._clip.ended()) {
          options._clip.play();
          options._audioFadeIn();
        }
      };
      ...
      options._onSeeked = () => {
        if (
          +_this.currentTime().toFixed(2) <
            options.start + options.audioFadeIn &&
    ...
    _update(options, updates) {
      if (
        updates.audioFadeIn >= 0 &&
        updates.audioFadeIn !== undefined &&
        updates.audioFadeIn !== options.audioFadeIn
      ) {
        options.audioFadeIn = updates.audioFadeIn;
      }
      if (
        updates.audioFadeOut >= 0 &&
        updates.audioFadeOut !== undefined &&
