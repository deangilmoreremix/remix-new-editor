...
import { BLEND_MODE } from '../../../constants/popcorn';
    ...
    _setup(options) {
      ...
      options._volumeEvent = () => {
        if (_this.muted()) {
          ...
          if (options._clip.volume) {
            const volume = (options.volume || options._natives.manifest.volume || 0) / 100;
            options._clip.volume(volume);
          }
        } else {
          options._clip.mute();
        }
