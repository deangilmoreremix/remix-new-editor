...
(function (Popcorn) {
  ...
  Popcorn.plugin('sequencer', {
    _setup(options) {
      ...
      options._onTimeUpdate = () => {
        ...
        if (+_this.currentTime().toFixed(2) > (options.end - options.audioFadeOut)
          && (+_this.currentTime().toFixed(2) < options.end) && !fade.fadeOut.isStarted) {
      ...
      options._audioFadeIn = () => {
        clearInterval(fade.fadeIn.interval);
        const step = 100;
        ...
        if (options.audioFadeIn > 0) {
          fade.fadeIn.time = _this.currentTime() - options.start;
          const startVolume = +((options.volume * fade.fadeIn.time)
            / options.audioFadeIn).toFixed(2);

          options._clip.volume((startVolume / 100));
          fade.fadeIn.currentVolume = startVolume;

          fade.fadeIn.interval = setInterval(() => {
            if (fade.fadeIn.time >= options.audioFadeIn
              || fade.fadeIn.currentVolume >= options.volume) {
              clearInterval(fade.fadeIn.interval);
              return options._clip.volume(options.volume / 100);
            }

            options._clip.volume(fade.fadeIn.currentVolume / 100);
            fade.fadeIn.currentVolume = +(fade.fadeIn.currentVolume + (options.volume * step)
              / (options.audioFadeIn * 1000)).toFixed(2);
            if (fade.fadeIn.currentVolume > options.volume) {
              fade.fadeIn.currentVolume = options.volume;
            }
          }, step);
        }
      ...
      options._audioFadeOut = () => {
        clearInterval(fade.fadeOut.interval);
        if ((_this.currentTime() > options.end)
          || options.audioFadeOut === undefined || options._clip.paused()) {
          return;
        }

        fade.fadeOut.isStarted = true;

        const step = 100;
        fade.fadeOut.time = _this.currentTime() - options.start;
        ...
        options._clip.volume((startVolume / 100));
        fade.fadeOut.currentVolume = startVolume;

        fade.fadeOut.interval = setInterval(() => {
          if (fade.fadeOut.time >= options.end
            || fade.fadeOut.currentVolume <= 0 || !fade.fadeOut.isStarted) {
            clearInterval(fade.fadeOut.interval);
            return options._clip.volume(0);
          }

          options._clip.volume(fade.fadeOut.currentVolume / 100);
          fade.fadeOut.currentVolume = +(fade.fadeOut.currentVolume - (options.volume * step)
            / (options.audioFadeOut * 1000)).toFixed(2);
          if (fade.fadeOut.currentVolume < 0) {
            fade.fadeOut.currentVolume = 0;
          }
        }, step);
      };
    ...
    _update(options, updates) {
      if (options.audioFadeIn >= 0 && updates.audioFadeIn !== undefined
        && updates.audioFadeIn !== options.audioFadeIn) {
        options.audioFadeIn = updates.audioFadeIn;
      }
      if (options.audioFadeOut >= 0 && updates.audioFadeOut !== undefined
      ...
      if (options.opacity !== undefined && options.opacity !== updates.opacity) {
