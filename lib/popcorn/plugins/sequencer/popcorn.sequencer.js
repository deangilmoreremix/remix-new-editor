...
import * as inputTypes from '../../../constants/forms';
import {
  BLEND_MODE,
  MUTE,
  VOLUME,
...
(function (Popcorn) {
  ...
  function isExternalVimeo(url) {
    return (/player.vimeo.com\/external\/\d+/).test(url);
  }
  ...
  function getDefaultOptionValue(key) {
    const { options } = Popcorn.manifest.sequencer;
  ...
  Popcorn.plugin('sequencer', {
    _setup(options) {
      ...
      options.displayLoading = () => {
        if (!options.waiting) {
          ...
          _waiting += 1;
          emitter.emit(emitterActions.SEQUENCES_LOADING);
        }
      };
      options.hideLoading = () => {
        if (options.waiting) {
          ...
          if (_waiting === 0) {
            ...
            emitter.emit(emitterActions.SEQUENCES_READY);
          }
        }
        emitter.emit(emitterActions.VIDEO_READY, {
          id: options.id,
          width: options._clip.media.videoWidth,
          height: options._clip.media.videoHeight,
        });
      };
      options.setZIndex = () => {
      ...
      options.readyEvent = () => {
        ...
        options._clip.media.style.objectFit = options.fill ? 'cover' : 'contain';
      ...
      options.addSource = () => {
        ...
        for (let i = 0; i < options.source.length; i += 1) {
          ...
          if (!options.notExternal && isExternalVimeo(value)) {
            value = options.preview || value;
          }
          const split = value.split('?');
          const querystring = split.length > 0 ? split[1] : '';
      ...
      options._isBuffering = () => {
        ...
        if (options._clip.media.readyState === 4) {
          return false;
        }
      ...
      options._onTimeUpdate = () => {
        ...
        if (+_this.currentTime().toFixed(2) > (options.end - options.audioFadeOut)
          && (+_this.currentTime().toFixed(2) < options.end) && !fade.fadeOut[`isStarted${options.id}`]) {
          options._audioFadeOut();
        }
      ...
      options._audioFadeIn = () => {
        clearInterval(fade.fadeIn[`interval${options.id}`]);
        const step = 100;

        if ((_this.currentTime() - options.start) >= options.audioFadeIn
          || options.audioFadeIn === undefined || options._clip.paused()) {
          return;
        }

        if (options.audioFadeIn > 0) {
          fade.fadeIn[`time${options.id}`] = _this.currentTime() - options.start;
          const startVolume = +((options.volume * fade.fadeIn[`time${options.id}`])
            / options.audioFadeIn).toFixed(2);

          options._clip.volume((startVolume / 100));
          fade.fadeIn[`currentVolume${options.id}`] = startVolume;

          fade.fadeIn[`interval${options.id}`] = setInterval(() => {
            if (fade.fadeIn[`time${options.id}`] >= options.audioFadeIn
              || fade.fadeIn[`currentVolume${options.id}`] >= options.volume) {
              clearInterval(fade.fadeIn[`interval${options.id}`]);
              return options._clip.volume(options.volume / 100);
            }

            options._clip.volume(fade.fadeIn[`currentVolume${options.id}`] / 100);
            fade.fadeIn[`currentVolume${options.id}`] = +(fade.fadeIn[`currentVolume${options.id}`] + (options.volume * step)
              / (options.audioFadeIn * 1000)).toFixed(2);
            if (fade.fadeIn[`currentVolume${options.id}`] > options.volume) {
              fade.fadeIn[`currentVolume${options.id}`] = options.volume;
            }
          }, step);
        }
      ...
      options._audioFadeOut = () => {
        clearInterval(fade.fadeOut[`interval${options.id}`]);
        if ((_this.currentTime() > options.end)
          || options.audioFadeOut === undefined || options._clip.paused()) {
          return;
        }

        fade.fadeOut[`isStarted${options.id}`] = true;

        const step = 100;
        fade.fadeOut[`time${options.id}`] = _this.currentTime() - options.start;

        let startVolume = +(options.volume - (options.volume - ((options.volume
          * (options.end - _this.currentTime())) / options.audioFadeOut))).toFixed(2);
        if (startVolume > options.volume) {
          startVolume = options.volume;
        }

        options._clip.volume((startVolume / 100));
        fade.fadeOut[`currentVolume${options.id}`] = startVolume;

        fade.fadeOut[`interval${options.id}`] = setInterval(() => {
          if (fade.fadeOut[`time${options.id}`] >= options.end
            || fade.fadeOut[`currentVolume${options.id}`] <= 0) {
            clearInterval(fade.fadeOut[`interval${options.id}`]);
            return options._clip.volume(0);
          }

          options._clip.volume(fade.fadeOut[`currentVolume${options.id}`] / 100);
          fade.fadeOut[`currentVolume${options.id}`] = +(fade.fadeOut[`currentVolume${options.id}`] - (options.volume * step)
            / (options.audioFadeOut * 1000)).toFixed(2);
          if (fade.fadeOut[`currentVolume${options.id}`] < 0) {
            fade.fadeOut[`currentVolume${options.id}`] = 0;
          }
        }, step);
      };
      ...
      options._pauseEvent = () => {
        if (!options._clip.paused()) {
          options._clip.pause();
          clearInterval(fade.fadeIn[`interval${options.id}`]);
          fade.fadeOut[`isStarted${options.id}`] = false;
          clearInterval(fade.fadeOut[`interval${options.id}`]);
        }
      };
      ...
      options._onSeeked = () => {
        if (+_this.currentTime().toFixed(2)
          < (options.start + options.audioFadeIn) && options.audioFadeIn > 0) {
          // if in the range fadeIn
          options._clip.volume(0);
        } else if (+_this.currentTime().toFixed(2) > (options.end - options.audioFadeOut)
          && (+_this.currentTime().toFixed(2) < options.end)
          && options.audioFadeOut > 0) {
          // if in the range fadeOut
          options._clip.volume(0);
        } else {
          options._clip.volume(options.volume / 100);
        }
      ...
      if (
        options.duration > 0
        && options.end > (options.start + options.duration - options.from)
      ) {
        options.end = options.start + options.duration - options.from;
      }
    ...
    _update(options, updates) {
      if (updates.audioFadeIn >= 0 && updates.audioFadeIn !== undefined
        && updates.audioFadeIn !== options.audioFadeIn) {
        options.audioFadeIn = updates.audioFadeIn;
      }
      if (updates.audioFadeOut >= 0 && updates.audioFadeOut !== undefined
        && updates.audioFadeOut !== options.audioFadeOut) {
        options.audioFadeOut = updates.audioFadeOut;
      }
      if (options.start >= 0 && updates.start !== undefined
        && updates.start !== options.start) {
        options.start = updates.start;
      }
      if (options.end && updates.end !== undefined && updates.end !== options.end) {
        options.end = updates.end;
      }
      if (updates.fill !== undefined && updates.fill !== options.fill) {
        options.fill = updates.fill;
        options._clip.media.style.objectFit = options.fill ? 'cover' : 'contain';
      }
      if (updates.duration !== undefined) {
        options.duration = updates.duration;
      }
      if (updates.from !== undefined && updates.from !== options.from) {
        options.from = updates.from;
      }
      if (options.end > (options.start + options.duration - options.from)) {
        options.end = options.start + options.duration - options.from;
      }
      ...
      if (updates.blendMode !== undefined) {
        options.blendMode = updates.blendMode;
        options._container.style.mixBlendMode = options.blendMode;
      }
      if (updates.opacity !== undefined) {
        options.opacity = updates.opacity;
