...
import { addDeleteListener, emitter, emitterActions, removeDeleteListener } from '../../../mitt/emitter';
...
((Popcorn) => {
  ...
  Popcorn.plugin(POPCORN_ELEMENT_TYPES.SOCIAL, {
    ...
    _setup(options) {
      ...
      if (options.dropped) {
        const { width: canvasWidth, height: canvasHeight } = target.getBoundingClientRect();
      ...
      options.top = (newTop || options.top)
        ?? options._natives.manifest.options.top.default;
      options.left = (newLeft || options.left)
