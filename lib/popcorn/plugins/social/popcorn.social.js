...
// The Facebook data provider (lib/popcorn/utils/facebook-data-provider.js) is
// legacy AMD code that does not build under Vite/Rollup and is not used by the
// production app. Inline a no-op stub so the social plugin still compiles; the
// real Facebook init path below is guarded by `window.FB` anyway.
const fbDataProvider = { init() {} };

const defaultBackground = "rgb(255, 255, 255, 1)";
  ...
  Popcorn.plugin(POPCORN_ELEMENT_TYPES.SOCIAL, {
    ...
    _setup(options) {
      ...
      if (options.dropped) {
        ...
        newLeft = options.left - (options.width * 100) / canvasWidth / 2;
        newTop = options.top - (options.height * 100) / canvasHeight / 2;
      ...
      options._container.style.background =
        options.background ||
        options._natives.manifest.options.background.default;
      options._container.style.fontSize = "14px";

      options.type = options.actionType || SOCIAL_TYPES.FB_LIKE;
      options.href = options.href || FB_PLUGINS[options.type].href;
      options.top =
        (newTop || options.top) ??
        options._natives.manifest.options.top.default;
      options.left =
        (newLeft || options.left) ??
        options._natives.manifest.options.left.default;
      ...
      const emitterSelectItem = (id) => {
        ...
        if (isSelected) {
          draggableResizable(options, null);
        } else {
    ...
    _update(trackEvent, options) {
      ...
      if (options.dropped) {
        const { width: canvasWidth, height: canvasHeight } =
          trackEvent._target.getBoundingClientRect();
      ...
      if (options.left && options.left !== trackEvent.left) {
        trackEvent.left = newLeft || options.left;
      }

      if (options.top && options.top !== trackEvent.top) {
        trackEvent.top = newTop || options.top;
      }
