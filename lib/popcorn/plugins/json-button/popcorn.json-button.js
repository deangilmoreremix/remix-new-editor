...
const phoneRegex = new RegExp('^(\\+[0-9\\s]*-?)?(\\([0-9\\s]*\\))?[0-9-.\\s]{10,14}$');
...
((Popcorn) => {
  ...
  function setClickAwayButton(options) {
    ...
    if (options[LINK_URL]
      && (validateUrl(options[LINK_URL]) || validatePhoneUrl(options[LINK_URL]))) {
  ...
  Popcorn.plugin(POPCORN_ELEMENT_TYPES.JSON_BUTTON, {
    manifest: {
      ...
      options: {
        ...
        [LINK_URL]: {
          ...
          featureLabels: {
            [FEATURES.REVOLUTION_CLICK_TO_PHONE_CALL]: LABEL_CLICK_TO_PHONE,
          },
          default: '',
        },
        [LEFT]: {
    ...
    _setup(options) {
      ...
      emitter.on(emitterActions.SELECT, id => {
        ...
        const deleteButton = options._container.querySelector('.delete-handle');
        ...
        if (!isSelected && deleteButton) {
          options._container.removeChild(deleteButton);
        }
