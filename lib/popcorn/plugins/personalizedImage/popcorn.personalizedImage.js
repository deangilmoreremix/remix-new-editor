...
import { draggableResizable, removeHandleElements ,removeDbClick } from '../../helpers';
import {
...
((Popcorn) => {
  let PLACEHOLDER_URL = 'https://cdn.vidcloud.io/src/plugins/personalizedImage/personalizedImage-portraitPlaceholder.png';
  let PLACEHOLDER_URL_CIRCLE = 'https://cdn.vidcloud.io/src/plugins/personalizedImage/personalizedImage-circlePlaceholder.png';
  PLACEHOLDER_URL_CIRCLE = '/static/images/circle-placeholder.svg'
  ...
  Popcorn.plugin('personalizedImage', {
    ...
    _setup(options) {
      ...
      options.imageshape = toFillOptions(IMAGESHAPE);
      ...
      emitter.on(emitterActions.SELECT, id => {
        ...
        if (isSelected) {
          draggableResizable({ ...options, _container, type: POPCORN_ELEMENT_TYPES.PERSONALIZED_IMAGE }, null);
        } else {
      ...
      _container.style.left = `${options.left || getDefaultOptionValue(LEFT)}%`;
      _container.style.top = `${options.top || getDefaultOptionValue(TOP)}%`;
      _container.style.width = `${options.width || getDefaultOptionValue(WIDTH)}%`;
      _container.style.height = `${options.height || getDefaultOptionValue(HEIGHT)}%`;
      _container.style.zIndex = options.zindex || getDefaultOptionValue(ZINDEX);
      ...
      if (_target) {
        ...
        if(options.imageshape == 'circle') {
          imageUrl = PLACEHOLDER_URL_CIRCLE;
        }
        if (arr[0] === 'd') {
    ...
    _update(trackEvent, options) {
      if(options.imageshape !== undefined) {
        trackEvent.imageshape = options.imageshape;
        let imageUrl = PLACEHOLDER_URL;
        if(options.imageshape == 'circle') {
          imageUrl =  PLACEHOLDER_URL_CIRCLE;
        }
