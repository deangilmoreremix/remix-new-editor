...
import EmbedLinkedinEngine from '../../../components/publisher/flows/linkedin/stages/EmbedEngine';
import EmbedLinkedinLocation from '../../../components/publisher/flows/linkedin/stages/EmbedLocation';
...
export const FACEBOOK_STAGES = [
  ...
  },
  {
    ...
    bootstrap: async ({ init, permissions, isAuthorized, nextStage, setStage, setLoading }) => {
      setLoading(true);
      await init();
      try {
...
export const LINKEDIN_STAGES = [
  ...
  },
  {
    ...
    bootstrap: async ({ init, isAuthorized, nextStage, setStage, setLoading }) => {
      setLoading(true);
      try {
        await init();
  ...
  },
  {
    ...
    bootstrap: async ({ project, fetchUserData, updateCampaign, setLoading }) => {
      setLoading(true);
      const userData = await fetchUserData();
      const postData = {
        title: project.name,
