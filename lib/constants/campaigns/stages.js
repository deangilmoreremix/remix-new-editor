...
export const LINKEDIN_STAGES = [
  ...
  },
  {
    ...
    bootstrap: async ({ init, isAuthorized, nextStage, setStage }) => {
      await init();
      try {
        if (await isAuthorized()) {
          await nextStage();
        } else {
          await setStage(LINKEDIN_LOGIN);
        }
      } catch (error) {
        console.error(error.message);
        await setStage(LINKEDIN_LOGIN);
  ...
  },
  {
    ...
    bootstrap: async ({ project, fetchUserData, updateCampaign }) => {
      const userData = await fetchUserData();
      const postData = {
        title: project.name,
        thumbnail: project.thumbnail,
