import { minimaxH3Demos as legacyMinimaxH3Demos, CATEGORY_ROUTES as legacyRoutes, getCreateTarget as legacyGetCreateTarget } from './minimaxH3Demos.js';
import { minimaxH3Demos, CATEGORY_ROUTES as minimaxRoutes, getCreateTarget as minimaxGetCreateTarget } from './beatapiMinimaxH3Demos.js';
...
// Tag-based studio overrides for demos whose category alone doesn't
// map cleanly to an ExampleGallery studio.
...
const EFFECTS_KEYWORDS = ['vfx', 'explosion', 'motion graphics', 'visual effects', 'action sequence'];
...
function isEffects(demo) {
  ...
  return (demo.tags || []).some((t) => EFFECTS_TAGS.has(t)) ||
    ...
    text.includes('visual effects') ||
    text.includes('action sequence') ||
    text.includes('tokusatsu') ||
    text.includes('parkour') ||
    text.includes('chase') ||
    text.includes('fight scene') ||
    text.includes('battle');
}

function mapToStudio(demo, routes) {
  // Effects should take priority over generic character/image matches
  // for demos that are clearly action/VFX oriented.
...
export const EXAMPLE_ASSETS = [
  ...legacyMinimaxH3Demos
    .map((demo) => createAsset(demo, 'minimax', legacyRoutes, legacyGetCreateTarget))
    .filter(Boolean),
  ...minimaxH3Demos
  ...
  // Avatar studio backfill — repos do not contain naturally-tagged avatar demos,
  // so we add curated entries so the studio can reach the cap.
