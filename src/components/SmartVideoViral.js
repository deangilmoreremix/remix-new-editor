...
function normalizeFlatSeedanceItem(item) {
  const videoUrl = (item.output && item.output.url) || '';
  const thumbUrl = (item.imgs && item.imgs[0] && item.imgs[0].url) || '';
  const promptText = item.prompt || '';
  const slug = item.slug || '';
  const detailHref = item.detailHref || '';
  const lang = item.sourceLanguage || '';
  const titleFromDetail = detailHref ? detailHref.replace(/^\/prompts\//, '').replace(/-/g, ' ').replace(/\b\w/g, c => c.toUpperCase()) : (slug || 'Seedance Prompt');
  const categories = guessCategories(promptText, titleFromDetail);
  const now = new Date().toISOString();
  return {
    imglumeId: slug || `seedance-flat-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`,
    ...
    source: {
      author: { handle: 'seedance', name: 'Seedance', link: detailHref ? `https://www.seedance2prompt.com${detailHref}` : '' },
    ...
    provenance: {
      discoveredBy: 'seedance2prompt',
      collection: 'seedance2prompt_imported',
      importedAt: now,
      updatedAt: now,
    },
    curation: {
      creator: 'Seedance2Prompt',
      url: detailHref ? `https://www.seedance2prompt.com${detailHref}` : '',
      recordUrl: detailHref ? `https://www.seedance2prompt.com${detailHref}` : '',
      license: 'CC-BY-4.0',
    },
    _source: 'seedance-flat',
  };
}
function guessCategories(promptText, titleText) {
  const text = `${promptText} ${titleText}`.toLowerCase();
  const map = [
    ['food-drink', /food|drink|restaurant|meal|chef|cuisine|recipe/],
    ['product-ads', /product|advert|commercial|brand|packaging|unboxing/],
    ['product-brand', /logo|brand|identity|typography|packaging|slogan/],
    ['character', /character|portrait|face|person|actor|girl|boy|man|woman/],
    ['cinematic', /cinematic|film|movie|scene|shot|dolly|zoom|bokeh|anamorphic/],
    ['animation', /animation|cartoon|anime|3d|render|c4d|blender|motion/],
    ['nature', /nature|forest|ocean|mountain|sky|sunset|waterfall|animal/],
    ['architecture', /architecture|building|interior|room|house|city|street/],
    ['travel', /travel|destination|vacation|resort|landscape|drone/],
    ['photography', /photo|photograph|portrait|studio|flash|lens|fujifilm|canon|nikon/],
    ['ui-graphic', /ui|graphic|interface|app|website|design|poster|banner/],
    ['camera-moves', /camera|pan|tilt|tracking|dolly|crane|steadicam|gimbal/],
    ['illustration-3d', /illustration|3d|sculpt|pixel|vector|sticker|toy|figure/],
    ['poster-design', /poster|cover|artwork|album|book|magazine|layout/],
    ['ugc', /ugc|talking head|selfie|vlog|creator|influencer|story/],
  ];
  const matched = [];
  for (const [cat, regex] of map) {
    if (regex.test(text)) matched.push(cat);
  }
  return matched.length ? matched : ['cinematic'];
}
...
export function SmartVideoViral() {
  ...
  const SEEDANCE_DATA_URL = '/seedance-prompts.json';
  ...
  async function loadData() {
    ...
    try {
      ...
      if (seedanceRes && seedanceRes.ok) {
        ...
        if (Array.isArray(seedanceJson)) {
          seedanceItems = seedanceJson.map(normalizeFlatSeedanceItem);
        } else if (seedanceJson.items && Array.isArray(seedanceJson.items)) {
