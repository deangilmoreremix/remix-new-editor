...
export function Sidebar(navigate) {
  ...
  const navItems = [
    ...
    { id: 'cinema', icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="2" width="20" height="20" rx="2.18" ry="2.18"/><line x1="7" y1="2" x2="7" y2="22"/><line x1="17" y1="2" x2="17" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><line x1="2" y1="7" x2="7" y2="7"/><line x1="2" y1="17" x2="7" y2="17"/><line x1="17" y1="17" x2="22" y2="17"/><line x1="17" y1="7" x2="22" y2="7"/></svg>', label: 'Cinema Studio' },
    { id: 'cinema-template', icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="2" y="2" width="20" height="20" rx="2.18" ry="2.18"/><line x1="7" y1="2" x2="7" y2="22"/><line x1="17" y1="2" x2="17" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/><line x1="2" y1="7" x2="7" y2="7"/><line x1="2" y1="17" x2="7" y2="17"/><line x1="17" y1="17" x2="22" y2="17"/><line x1="17" y1="7" x2="22" y2="7"/></svg>', label: 'Cinema Template Studio' },
    ...
    { id: 'academy', icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 10L12 5 2 10l10 5 10-5z"/><path d="M6 12v5c0 1 2.7 2.5 6 2.5s6-1.5 6-2.5v-5"/></svg>', label: 'Smart Video Academy' },
    { id: 'viral', icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 2L2 7l10 5 10-5-10-5z"/><path d="M2 17l10 5 10-5"/><path d="M2 12l10 5 10-5"/></svg>', label: 'Smart Video Viral' },
    ...
    { id: 'content-library', icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 19a2 2 0 01-2 2H4a2 2 0 01-2-2V5a2 2 0 012-2h5l2 3h9a2 2 0 012 2z"/></svg>', label: 'Content' },
...
{ id: 'pexels-media', icon: '<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"/><polygon points="16.24 7.76 14.12 14.12 7.76 16.24 9.88 9.88 16.24 7.76"/></svg>', label: 'Stock Media' },
  ...
  // Mobile menu toggle button. On desktop the sidebar is always visible (via
  // the `hidden md:flex` class), so this button is `md:hidden` and only
  // matters on small screens, where it slides the sidebar in as a fixed
  // overlay. main.js mounts it on <body>.
  const toggleBtn = document.createElement('button');
  toggleBtn.type = 'button';
  toggleBtn.setAttribute('aria-label', 'Toggle navigation menu');
  toggleBtn.className = 'md:hidden fixed top-3 left-3 z-[60] w-10 h-10 rounded-xl bg-panel-bg border border-white/10 text-white flex items-center justify-center';
  ...
  backdrop.className = 'md:hidden fixed inset-0 z-40 bg-black/60 hidden';

  let sidebarOpen = false;
  const openSidebar = () => {
    sidebarOpen = true;
    element.style.display = 'flex';
    element.style.paddingLeft = '52px';
    element.classList.add('fixed', 'inset-y-0', 'left-0', 'z-50');
  ...
  const closeSidebar = () => {
    ...
    element.classList.remove('fixed', 'inset-y-0', 'left-0', 'z-50');
  ...
  element.addEventListener('route-changed', closeSidebar);
  window.addEventListener('navigate', closeSidebar);

  document.body.appendChild(backdrop);

  return { sidebar: element, toggleBtn };
}
