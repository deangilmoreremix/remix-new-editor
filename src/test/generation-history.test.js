 ...
 * - saveGeneration stores request_id in the entry for webhook correlation
 * - deleteGeneration removes entries by URL from the correct localStorage key
 * - loadGenerationHistory merges localStorage entries with a fallback shape
...
describe('loadGenerationHistory', () => {
  ...
  test('merges cloud and local, deduplicating by URL', async () => {
    // Local entry
    saveGeneration({ studio: 'image', type: 'image', url: 'https://a.com/shared.png', prompt: 'Shared' });
    saveGeneration({ studio: 'image', type: 'image', url: 'https://b.com/local.png', prompt: 'Local' });

    // Cloud mock returns the shared URL + a cloud-only URL
    fetchMock.mockResolvedValueOnce({
      ok: true,
