/**
 * Shared model selector UI utilities for studios.
 *
 * Provides:
...
export function renderProviderSidebar(availableProviders, selectedProvider) {
...
export function renderModelList(models, selectedModelId, showProviderName) {
  if (models.length === 0) {
...
// ---------------------------------------------------------------------------
// Convenience: mount split-pane dropdown into a container element
// ---------------------------------------------------------------------------
...
export function mountModelSelector(container, options = {}) {
  ...
  dropdown.innerHTML = `
    <div class="flex w-full rounded-2xl border border-white/5 bg-black/40 backdrop-blur-xl overflow-hidden" style="min-height: 320px; max-height: 420px;">
      ${renderProviderSidebar(availableProviders, selectedProvider, onSelectProvider)}
      <div class="flex flex-col flex-1 min-w-0 p-3 gap-2.5">
        ${renderSearchBar()}
        ${renderModelList(filteredModels, selectedModelId, showProviderName, onSelectModel)}
      </div>
    </div>
  `;
  ...
  const providerButtons = dropdown.querySelectorAll('[data-provider]');
  providerButtons.forEach((btn) => {
    btn.addEventListener('click', () => {
      const provider = btn.dataset.provider;
      onSelectProvider(provider === 'all' ? 'all' : provider);
    });
  });

  // Wire up model items
  const modelItems = dropdown.querySelectorAll('[data-model-id]');
