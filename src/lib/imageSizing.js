export const T2I_DIMENSION_RATIOS = Object.freeze(['1:1', '16:9', '9:16', '4:3', '3:2', '21:9']);
export const I2I_DIMENSION_RATIOS = Object.freeze(['1:1', '16:9', '9:16']);
...
export const resolveImageDimensions = (model, aspectRatio) => {
  ...
  const scaleStep = lcm(widthStep / gcd(widthStep, ratio.width), heightStep / gcd(heightStep, ratio.height));
  const minimumScale = Math.ceil(Math.max(capability.width.minValue / ratio.width, capability.height.minValue / ratio.height) / scaleStep) * scaleStep;
  const maximumScale = Math.floor(Math.min(capability.width.maxValue / ratio.width, capability.height.maxValue / ratio.height) / scaleStep) * scaleStep;
...
export const buildImageSizePayload = (model, aspectRatio) => {
  const capability = getImageSizeCapability(model);
  if (capability.type === 'aspect_ratio') {
    return capability.input.enum.includes(aspectRatio) ? { aspect_ratio } : {};
  }
