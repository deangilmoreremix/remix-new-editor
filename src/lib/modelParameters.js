...
function clampNumber(value, schema) {
  ...
  return ['int', 'integer'].includes(schema.type) && !fractionalStep ? Math.round(clamped) : clamped;
}
...
function normalizeValue(value, schema, { includeEmpty = false } = {}) {
  ...
  if (schema.type === 'object') {
    ...
    if (!includeEmpty && identifyingKeys.length > 0 && identifyingKeys.every((key) => !String(value[key] ?? '').trim())) {
      return undefined;
    }
    const normalized = {};
...
export function createModelParameterValues(model, previousValues = {}) {
  ...
  for (const { key, schema } of getSupplementalModelInputs(model)) {
    const source = Object.hasOwn(previousValues, key) ? previousValues[key] : defaultValue(schema);
