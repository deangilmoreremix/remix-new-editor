...
const requiredImage = (field, label) =>
  Object.freeze({
    field,
    label,
    required: true,
...
const CONTRACTS = Object.freeze({
  't2i:flux-pulid': Object.freeze({ primaryImageRequired: true, promptRequired: true }),
  'i2i:flux-pulid': Object.freeze({ promptRequired: true }),
  't2i:flux-redux': Object.freeze({ primaryImageRequired: true, promptRequired: true }),
  'i2i:flux-redux': Object.freeze({ promptRequired: true }),
  'i2i:ai-dress-change': Object.freeze({
    primaryImageLabel: 'Model image',
    auxiliaryImages: Object.freeze([requiredImage('garment_image_url', 'Garment image')]),
  }),
  ...
  'i2i:ai-object-eraser': Object.freeze({
    ...
    auxiliaryImages: Object.freeze([requiredImage('mask_image_url', 'Mask image')]),
  }),
  ...
  'i2i:add-image-watermark': Object.freeze({
    primaryImageLabel: 'Source image',
    auxiliaryImages: Object.freeze([requiredImage('watermark_image_url', 'Watermark image')]),
  }),
});

export function getImageInputContract(model, mode) {
  const contractKey = `${mode}:${model?.id}`;
  const patch = Object.hasOwn(CONTRACTS, contractKey) ? CONTRACTS[contractKey] : {};
  const isTextToImage = mode === 't2i';
  const promptRequired = patch.promptRequired ?? model?.promptRequired ?? isTextToImage;
  return {
    ...BASE_CONTRACT,
...
export function getAuxiliaryImageInputs(model, mode) {
  ...
  return [requiredImage(model.swapField, 'Swap face image'), ...configured];
}

export function getImageInputValidationError(model, mode, { prompt = '', primaryImageUrls = [], auxiliaryImageUrls = {} } = {}) {
  ...
  if (contract.promptRequired && !hasText(prompt)) {
    return contract.promptField === 'scene_description' ? 'Please enter a scene description.' : 'Please enter a prompt.';
  }
  const missingImage = getAuxiliaryImageInputs(model, mode).find(({ field, required }) => required && !hasText(auxiliaryImageUrls[field]));
  return missingImage ? `Please upload a ${missingImage.label.toLowerCase()}.` : null;
}
