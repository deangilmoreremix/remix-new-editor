...
function videoVariantKey(model) {
  ...
  return effectOptions?.length ? `${endpoint}\u0000${effectOptions.join('\u0000')}` : endpoint;
}

const PREFERRED_IMAGE_VARIANTS = {
  'flux-2-klein': { t2i: 'flux-2-klein-4b', i2i: 'flux-2-klein-4b-edit' },
  ...
  'qwen-image-2.0': { t2i: 'qwen-image-2.0', i2i: 'qwen-image-2.0-edit' },
  'qwen-plus': { i2i: 'qwen-image-edit-plus' },
  qwen3: { t2i: 'qwen3-text-to-image', i2i: 'qwen3-image-to-image' },
  'seedream-5.0': { t2i: 'bytedance-seedream-v5.0', i2i: 'bytedance-seedream-v5.0-edit' },
  'wan2.7': { t2i: 'wan2.7-text-to-image', i2i: 'wan2.7-image-edit' },
};

const PREFERRED_VIDEO_VARIANTS = {
  'kling-v3': { t2v: 'kling-v3.0-standard-text-to-video', i2v: 'kling-v3.0-standard-image-to-video', v2v: 'kling-v3.0-std-motion-control' },
  'seedance-2.5': { t2v: 'seedance-2.5-text-to-video', i2v: 'seedance-2.5-image-to-video', v2v: 'seedance-2.5-video-edit' },
  'wan-3': { t2v: 'wan3.0-text-to-video', i2v: 'wan3.0-image-to-video' },
  'flux-3': { t2v: 'flux-3-text-to-video', i2v: 'flux-3-image-to-video', v2v: 'flux-3-video-extend' },
  'gemini-omni': { t2v: 'gemini-omni-flash-1-1-text-to-video', i2v: 'gemini-omni-flash-1-1-image-to-video', v2v: 'gemini-omni-flash-1-1-edit' },
  'ltx-2.5': { t2v: 'ltx-2.5-text-to-video', i2v: 'ltx-2.5-image-to-video' },
  'minimax-h3': { t2v: 'minimax-h3-text-to-video', i2v: 'minimax-h3-image-to-video' },
};
...
function videoSeriesVersion(family) {
  const match = family.id.match(/^(kling-v|seedance-|wan-|veo-|pixverse-|vidu-q|ltx-|happy-horse-|minimax-hailuo-)(\d+(?:\.\d+)?)/);
...
function buildCatalog(modeLists, config) {
  ...
  const families = [...familyMap.values()].map((family) => {
    ...
    const namingEntry = [...family.rawVariants[preferredMode]].sort((a, b) => b.freshness - a.freshness)[0];
    ...
    family.searchText = [
      family.id,
      family.name,
      ...Object.values(family.rawVariants).flatMap((variants) => variants.flatMap(({ model }) => [model.id, model.name])),
    ]
      .join(' ')
      .toLowerCase();
  ...
  if (config.seriesVersion) {
    ...
    seriesGroups.forEach((entries) => {
      const freshnessSlots = entries.map(({ family }) => family.freshness).sort((a, b) => b - a);
      entries
        .sort((a, b) => b.version - a.version || b.family.freshness - a.family.freshness)
        .forEach(({ family }, index) => {
          family.sortFreshness = freshnessSlots[index];
        });
    });
  }
  families.sort((a, b) => b.sortFreshness - a.sortFreshness || a.name.localeCompare(b.name));
...
export function getFamilyVariant(catalog, familyOrId, mode, currentVariantId = null) {
  const family = typeof familyOrId === 'string' ? catalog.familyById.get(familyOrId) : familyOrId;
  const variants = family?.variants[mode] || [];
  if (variants.length === 0) return null;

  const currentFamily = currentVariantId ? catalog.familyByVariantId.get(currentVariantId) : null;
...
export function getImageReferenceVariant(catalog, familyOrId, currentVariantId) {
  const family = typeof familyOrId === 'string' ? catalog.familyById.get(familyOrId) : familyOrId;
  const currentVariant = catalog.variantById.get(currentVariantId);
  if (family && catalog.familyByVariantId.get(currentVariantId)?.id === family.id && getModelMediaCapabilities(currentVariant?.model).image.maxItems > 0) {
...
function pickerDisplayName(model, family, isAlias) {
  ...
  return suffix !== baseName ? (suffix ? `${family.name} ${suffix}` : family.name) : baseName;
}
...
function aliasProfile(modelId, alias) {
  ...
  const tokens = (value) => value.toLowerCase().replace(/\bv(?=\d)/g, '').split(/[^a-z0-9]+/).filter(Boolean);
  const aliasTokens = new Set(tokens(alias));
  return tokens(modelId).filter((token) => !aliasTokens.has(token) && !ignoredTokens.has(token)).join('-');
}
...
function formatAliasProfile(profile) {
  ...
  return profile
    .split('-')
    .filter(Boolean)
    .map((token) => special[token] || (/^\d+b$/.test(token) ? token.toUpperCase() : token[0].toUpperCase() + token.slice(1)))
    .join(' ');
}
...
function pickerContractKey(model) {
  const inputs = Object.fromEntries(
    Object.entries(model.inputs || {})
      .filter(
        ([name, input]) =>
          !PICKER_MEDIA_FIELDS.has(name) && !['image', 'video', 'audio'].includes(input?.field),
      )
...
function buildModelPickerEntries(catalog, modes, aliases) {
  ...
  for (const family of catalog.families) {
    ...
    for (const entry of familyEntries.values()) {
      entry.defaultVariant = modes.map((mode) => entry.variantsByMode[mode]).find(Boolean) || null;
...
export const imageModelPickerEntries = buildModelPickerEntries(imageModelCatalog, ['t2i', 'i2i'], IMAGE_FAMILY_ALIASES);
export const videoModelPickerEntries = buildModelPickerEntries(videoModelCatalog, ['t2v', 'i2v', 'v2v'], VIDEO_FAMILY_ALIASES);
...
export const imageModelPickerEntryByVariantId = indexModelPickerEntries(imageModelPickerEntries, imageModelCatalog);
export const videoModelPickerEntryByVariantId = indexModelPickerEntries(videoModelPickerEntries, videoModelCatalog);
