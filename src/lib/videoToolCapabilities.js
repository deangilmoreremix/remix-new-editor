...
const MEDIA_CONFIG = {
  image: {
    ...
    fields: ['images_list', 'image_urls', 'reference_images', 'image_url', 'start_image_url', 'first_image_url'],
  ...
  video: {
    ...
    fields: ['videos_list', 'video_files', 'video_urls', 'reference_videos', 'video_url'],
  ...
  audio: {
    ...
    fields: ['audios_list', 'audio_files', 'audio_urls', 'reference_audios', 'audio_url'],
    singularParam: 'audio_url',
    pluralParam: 'audios_list',
  },
};

const REQUIRED_EDIT_TOOL_IDS = new Set([
  'kling-o1-video-edit',
...
const REQUIRED_EXTEND_TOOL_IDS = new Set([
...
const HAPPY_HORSE_EDIT_TOOL_IDS = new Set([
  'happy-horse-1-video-edit-1080p',
  'happy-horse-1-video-edit-720p',
  'happy-horse-1.1-video-edit-1080p',
  'happy-horse-1.1-video-edit-720p',
...
for (const id of REQUIRED_EDIT_TOOL_IDS) {
  VIDEO_TOOL_OVERRIDES[id] = { hasPrompt: true, promptRequired: true, operation: 'edit' };
}

for (const id of REQUIRED_EXTEND_TOOL_IDS) {
  VIDEO_TOOL_OVERRIDES[id] = { hasPrompt: true, promptRequired: true, operation: 'extend' };
}
...
for (const id of KLING_EDIT_TOOL_IDS) {
  ...
  VIDEO_TOOL_OVERRIDES[id] = { ...VIDEO_TOOL_OVERRIDES[id], inputs, payloadDefaults: { images_list: [] } };
}

Object.assign(VIDEO_TOOL_OVERRIDES, {
  'video-watermark-remover': { hasPrompt: false, operation: 'watermark' },
  'ai-video-upscaler': {
    hasPrompt: false,
    operation: 'upscale',
    summary: 'Upscale the source video to the selected resolution. This integration exposes target resolution and audio preservation; it does not expose an output-format setting.',
    inputs: {
      resolution: { title: 'Target resolution', type: 'string', enum: ['720p', '1080p', '2k', '4k'], default: '720p', configurable: true },
      copy_audio: { title: 'Keep audio', type: 'boolean', default: true, configurable: true },
    },
  },
  ...
  'ltx-2.3-video-extend': {
    ...
    operation: 'extend',
    estimateCost: true,
    inputs: { duration: { title: 'Extend duration', type: 'integer', enum: Array.from({ length: 20 }, (_, index) => index + 1), default: 5, configurable: true } },
  },
  'seedance-v2.0-extend': { hasPrompt: true, promptRequired: false, operation: 'extend' },
  'seedance-2-extend': { hasPrompt: true, promptRequired: false, operation: 'extend' },
  'seedance-2-vip-extend': { hasPrompt: true, promptRequired: false, operation: 'extend' },
  'seedance-2-vip-extend-1080p': { hasPrompt: true, promptRequired: false, operation: 'extend' },
  'veo3.1-extend-video': { hasPrompt: true, promptRequired: true, operation: 'extend' },
  'veo3.1-4k-video': {
    hasPrompt: false,
    promptRequired: false,
    operation: 'upscale',
  ...
  'grok-imagine-extend': {
    ...
    promptRequired: true,
    operation: 'extend',
    inputs: { extend_times: { title: 'Extend duration', type: 'integer', enum: [6, 10], default: 6, configurable: true } },
  },
  'runway-aleph-v2v': {
    ...VIDEO_TOOL_OVERRIDES['runway-aleph-v2v'],
    inputs: {
      aspect_ratio: { title: 'Aspect ratio', type: 'string', enum: ['9:16', '16:9', '1:1', '4:3', '3:4', '21:9'], default: '16:9', configurable: true },
    },
  },
  'pixverse-v6-extend': {
    ...VIDEO_TOOL_OVERRIDES['pixverse-v6-extend'],
    inputs: {
      resolution: { title: 'Resolution', type: 'string', enum: ['360p', '540p', '720p', '1080p'], default: '720p', configurable: true },
      duration: { title: 'Duration', type: 'integer', enum: Array.from({ length: 15 }, (_, index) => index + 1), default: 5, configurable: true },
      generate_audio_switch: { title: 'Generate audio', type: 'boolean', default: false, configurable: true },
      negative_prompt: { title: 'Negative prompt', type: 'string', configurable: true, optional: true },
      style: { title: 'Style', type: 'string', enum: ['anime', '3d_animation', 'clay', 'comic', 'cyberpunk'], configurable: true, optional: true },
    },
  },
});
...
VIDEO_TOOL_OVERRIDES['seedance-v1.5-pro-video-extend'] = {
  ...VIDEO_TOOL_OVERRIDES['seedance-v1.5-pro-video-extend'],
  estimateCost: true,
  payloadDefaults: { resolution: '720p', duration: 5, generate_audio: true, camera_fixed: false },
};
...
for (const id of HAPPY_HORSE_EDIT_TOOL_IDS) {
  VIDEO_TOOL_OVERRIDES[id] = {
    ...VIDEO_TOOL_OVERRIDES[id],
    estimateCost: true,
    summary: 'Edit a source video with a required natural-language instruction.',
    guidance: 'Upload one source video that meets the listed requirements, then describe the edit you want.',
...
const CONTINUATION_FAMILIES = Object.freeze({
  seedance: {
    ...
    emptySourceMessage: 'Select a completed Seedance 2.0 video before continuing.',
  },
  veo31: {
    sourceModelIds: Object.freeze([
  ...
  grok: {
    sourceModelIds: Object.freeze(['grok-imagine-text-to-video', 'grok-imagine-image-to-video']),
    sourceLabel: 'Grok Imagine source video',
    emptySourceMessage: 'Select a completed Grok Imagine video before extending it.',
  },
});
...
const CONTINUATION_TARGETS = Object.freeze({
  ...
  'veo3.1-extend-video': { family: 'veo31', requiredSourceResolution: '720p', sourceLabel: 'Veo 3.1 or Fast 720p source video', emptySourceMessage: 'Select a completed 720p Veo 3.1 or Veo 3.1 Fast video before extending it.' },
  'veo3.1-4k-video': { family: 'veo31', sourceLabel: 'Veo 3.1 or Fast source video', emptySourceMessage: 'Select a completed Veo 3.1 or Veo 3.1 Fast video before creating a 4K version.' },
...
function getVideoToolCapabilities(model) {
  ...
  return { ...model, ...override, inputs: mergeInputs(model.inputs, override.inputs) };
}
...
export function getVideoToolPresentation(model) {
  ...
  const showPrompt = capabilities.hasPrompt ?? Boolean(capabilities.inputs?.prompt);
  ...
  return {
    actionLabel: capabilities.actionLabel || ACTION_LABELS[operation] || ACTION_LABELS.process,
    promptPlaceholder: promptRequired ? promptLabel : `${promptLabel} (optional)`,
    promptRequired,
    showPrompt,
    summary: capabilities.summary || capabilities.description || `${capabilities.name || 'The selected tool'} is ready to process the video.`,
...
export function resolveVideoUploadTransition({ mode, currentModel, defaultModel }) {
  const preservesSelection =
    (mode === 'v2v' && Boolean(currentModel)) || Boolean(currentModel?.inputs?.video_files);

  if (preservesSelection) {
    return { clearImage: false, clearPrompt: false, mode, model: currentModel };
  }

  return { clearImage: mode === 'i2v', clearPrompt: true, mode: 'v2v', model: defaultModel };
}
...
export function buildVideoToolPayload(model, params = {}) {
  ...
  const payload = { [videoField]: params.video_url, ...getVideoToolPayloadDefaults(model) };
  ...
  return { ...payload, ...serializeVideoToolOptions(model, params.options) };
}
...
export function getContinuationConfig(modelOrId) {
  ...
  const capabilities = getVideoToolCapabilities(model);
  return { ...CONTINUATION_FAMILIES[target.family], ...target, promptRequired: Boolean(capabilities?.promptRequired) };
}
...
function normalizeRequestId(value) {
  ...
  if (/^\d+$/.test(requestId)) {
    ...
    if (Number.isFinite(numericId) && numericId >= MIN_CLIENT_TIMESTAMP && numericId < MAX_CLIENT_TIMESTAMP) {
      return null;
    }
  }
  return requestId;
...
export function getCompatibleContinuationSources(modelOrId, history = []) {
  ...
  for (const entry of history) {
    ...
    if (config.requiredSourceResolution) {
      const sourceResolution = entry.resolution || config.sourceDefaultResolutions?.[entry.model];
      if (String(sourceResolution).toLowerCase() !== config.requiredSourceResolution.toLowerCase()) {
        continue;
      }
    }
    sources.push(entry.requestId === requestId ? entry : { ...entry, requestId });
  }
  return sources;
}
