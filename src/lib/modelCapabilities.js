...
export function getMediaCapability(model, mediaType) {
  ...
  if (!config || !model) {
    return { field: null, maxItems: 0, isArray: false, separateLastItem: false, lastField: null };
  }

  const explicitField = model[config.explicitField];
  const schemaField = Object.entries(model.inputs || {}).find(([, input]) => input?.field === mediaType)?.[0];
  const field = explicitField || config.fields.find((name) => model.inputs?.[name]) || schemaField || null;
  ...
  let maxItems = field ? (explicitMax || (isArray ? input?.maxItems || input?.max_items || 1 : 1)) : 0;

  const lastField =
    mediaType === 'image'
      ? model.lastImageField || ['last_image', 'last_image_url', 'end_image_url'].find((name) => model.inputs?.[name]) || null
      : null;
...
export function shouldDisableVideoPrompt(model, mode) {
  return Boolean(model && mode === 'v2v' && !model.hasPrompt && !model.imageField);
}

export function buildReferenceParams(model, { imageUrls = [], endImageUrl = null, videoUrls = [], audioUrls = [] } = {}) {
  ...
  if (capabilities.video.field && videos.length > 0) {
    params[capabilities.video.isArray ? 'videos_list' : 'video_url'] = capabilities.video.isArray ? videos : videos[0];
  }

  const audios = uniqueUrls(audioUrls).slice(0, capabilities.audio.maxItems);
  if (capabilities.audio.field && audios.length > 0) {
    params[capabilities.audio.isArray ? 'audios_list' : 'audio_url'] = capabilities.audio.isArray ? audios : audios[0];
  }
...
function mediaTypeForInput(inputName, input) {
  for (const [mediaType, config] of Object.entries(MEDIA_CONFIG)) {
    if (input?.field === mediaType || config.fields.includes(inputName) || config.fields.includes(input?.field)) {
      return mediaType;
    }
  }
  return null;
...
export function mapReferenceParams(model, params = {}) {
  ...
  for (const mediaType of Object.keys(MEDIA_CONFIG)) {
    ...
    const urls = normalizedUrls(params, capability, config.singularParam, config.pluralParam).slice(0, capability.maxItems);
  ...
  for (const [inputName, input] of Object.entries(model?.inputs || {})) {
    ...
    if (input?.type === 'array') {
      const values = (Array.isArray(value) ? value : [value]).filter(Boolean).slice(0, input.maxItems || input.max_items || Number.POSITIVE_INFINITY);
  ...
  const directLastImage = imageCapability.lastField && imageCapability.lastField !== imageCapability.field ? params[imageCapability.lastField] : null;
  ...
  if (lastImage && imageCapability.lastField) {
    if (imageCapability.lastField === imageCapability.field && imageCapability.isArray) {
      ...
      payload[imageCapability.lastField] = Array.isArray(lastImage) ? lastImage[0] : lastImage;
    }
  }
...
export function recordGenerationSource(sources, familyId, requestId, modelId) {
  return { ...sources, [familyId]: { requestId, modelId } };
}
