/**
 * MODEL CAPABILITIES
 * Maps model input schemas to media capabilities for upload routing,
 * payload construction, and validation.
 *
 * Ported from upstream Open-Generative-AI packages/studio/src/modelCapabilities.js
 * Adapted for local vanilla JS architecture.
 */
...
export function getMediaCapability(model, mediaType) {
  ...
  let maxItems = field
    ? explicitMax ||
      (isArray
        ? input?.maxItems || input?.max_items || 1
        : 1)
    : 0;
...
export function mapReferenceParams(model, params = {}) {
  ...
  // media type (for example Wan: an optional image_url anchor plus reference
