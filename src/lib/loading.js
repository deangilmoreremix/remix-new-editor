...
/**
 * Create a full-coverage loading overlay.
 *
 * @param {string} [message='Loading...']
 * @returns {{ show(): void, hide(): void, element: HTMLDivElement }}
 */
export function createLoadingOverlay(message = 'Loading...') {
  ...
  return { show: overlay.show, hide: overlay.hide, element: overlay };
}

/**
 * Create a compact inline loader with optional message.
 *
...
/**
 * Create an animated progress bar with percentage label.
 *
 * @param {number} [initialProgress=0]
 * @param {string} [width='']
 *   Optional CSS width constraint (e.g. '100%', 'max-w-xs').
 *   Defaults to 'w-full max-w-xs'.
 * @returns {HTMLDivElement}
 */
export function createProgressBar(initialProgress = 0, width = '') {
  const widthClass = width || 'w-full max-w-xs';
  ...
  container.innerHTML = `
    ...
    <div class="flex justify-between mt-1">
      <span class="text-xs text-secondary progress-label">Generating...</span>
      <span class="text-xs text-primary font-mono progress-text">${Math.round(initialProgress)}%</span>
    </div>
  `;
