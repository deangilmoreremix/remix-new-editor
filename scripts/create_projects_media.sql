-- Create projects + media tables on the REMOTE project so the app's
-- hybrid-supabase data layer (timeline editor, drag-drop, upload pipeline)
-- stores real server-side data instead of falling back to localStorage.
...
-- let the client read/write real data. Tighten later if per-user isolation
-- is needed (e.g. add user_key + x-user-key header in the supabase client).
