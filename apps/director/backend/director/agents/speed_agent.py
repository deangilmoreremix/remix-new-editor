...
SPEED_AGENT_PARAMETERS = {
    ...
    "properties": {
        ...
        "speed": {"type": "number", "description": "Playback speed multiplier", "default": 1.5},
    },
...
class SpeedAgent(BaseAgent):
    def __init__(self, session: Session, **kwargs):
        ...
        self.description = (
            "Adjust the playback speed of a video. Real-time SDK speed control may be "
            "limited, so this is approximated via a best-effort timeline pass and "
            "degrades gracefully when the capability is not supported."
        )
        self.parameters = SPEED_AGENT_PARAMETERS
        super().__init__(session=session, **kwargs)

    def run(self, collection_id: str, video_id: str, speed: float = 1.5, *args, **kwargs):
        try:
            self.output_message.actions.append(f"Adjusting speed to {speed}x...")
            ...
            try:
                ...
                logger.warning("Clip speed not supported; proceeding with default speed.")
